    ...
    if(MASTER_SITE != DOMAIN_INSET) { 
        ...
        if(is_array($arr_master) /*&& count($arr_master)*/) {
			if(defined("AUTH_USERNAME") && strlen(AUTH_USERNAME) && defined("AUTH_PASSWORD") && strlen(AUTH_PASSWORD)) {
	            ...
	            $json_slave = @file_get_contents("http://" . DOMAIN_INSET . FF_SITE_PATH . REAL_PATH . "/updater/check/file.php" . $strContestPath . "?s=" . urlencode(DOMAIN_INSET), false, $context);
	            if($json_slave === false && strpos(DOMAIN_INSET, "www.") === false) {
	                $json_slave = @file_get_contents("http://www." . DOMAIN_INSET . FF_SITE_PATH . REAL_PATH . "/updater/check/file.php" . $strContestPath . "?s=" . urlencode("www." . DOMAIN_INSET), false, $context);
	            }
			} else {
				$context = stream_context_create(array(
					"ssl"=>array(
