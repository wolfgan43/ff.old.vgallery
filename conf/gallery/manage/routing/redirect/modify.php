...
if(isset($_REQUEST["frmAction"]) && isset($_REQUEST["setvisible"]) && $_REQUEST["keys"]["ID"] > 0) {
    $sSQL = "UPDATE `cache_page_redirect`
                    ...
                    WHERE 
                        `cache_page_redirect`.`ID` = " . $db_gallery->toSql($_REQUEST["keys"]["ID"], "Number");
...
if($_REQUEST["keys"]["ID"] > 0) {
	$sSQL = "SELECT * FROM cache_page_redirect WHERE ID = " . $db_gallery->toSql($_REQUEST["keys"]["ID"], "Number");
	$db_gallery->query($sSQL);
	if($db_gallery->nextRecord()) {
		$all_phone = $db_gallery->getField("all_phone", "Number", true)
		$section_blocks = $db_gallery->getField("section_blocks", "Text", true);
		$layout_blocks = $db_gallery->getField("layout_blocks", "Text", true);
		$ff_blocks = $db_gallery->getField("ff_blocks", "Text", true);
    ...
    if(strlen($action)) {
    	switch($action) {
    		...
    		case "confirmdelete":
    			...
    			@unlink(CM_CACHE_DISK_PATH . "/redirect/" . $src_host . ".php");
    			@unlink(CM_CACHE_DISK_PATH . "/redirect/" . $src_host . ".rule.php");
    			break;
    			//$strError = make_routing_table_file();
    			break;
