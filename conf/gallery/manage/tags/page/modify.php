...
if($db_gallery->nextRecord()) {
    do {
        $arrSearchTag[$db_gallery->getField("ID_lang", "Number", true)][$db_gallery->getField("name", "Text", true)] = $db_gallery->getField("ID", "Number", true);
...
if(isset($_REQUEST["keys"]["code"]) && $_REQUEST["keys"]["code"] > 0) {
    $sSQL_string = "search_tags_page.code = " . $db_gallery->toSql($_REQUEST["keys"]["code"], "Number");
} elseif(isset($_REQUEST["keys"]["ID"]) && $_REQUEST["keys"]["ID"] > 0) {
    $sSQL_string = "search_tags_page.ID = " . $db_gallery->toSql($_REQUEST["keys"]["ID"], "Number");
}
        ...
        do {
            ...
            if(array_key_exists($ID_lang, $arrLang))
            {
                $arrTag[$arrLang[$ID_lang]] = array("ID_lang" => $ID_lang
                                                        ...
                                                        , "title" => $db_gallery->getField("meta_title", "Text", true)
                                                        , "description" => $db_gallery->getField("meta_description", "Text", true)
                                                        , "keywords" => $db_gallery->getField("keywords", "Text", true)
                                                        , "h1" => $db_gallery->getField("h1", "Text", true)
                                                        , "h2" => $db_gallery->getField("h2", "Text", true)
                                                        , "pre_content" => $db_gallery->getField("pre_content", "Text", true)
                                                        , "code" => $db_gallery->getField("code", "Number", true)
                                                        , "post_content" => $db_gallery->getField("post_content", "Text", true)
                                                    );
            }
        } while ($db_gallery->nextRecord());
    }
...
if($db_gallery->nextRecord()) {
    ...
    do {
        $sSQL2 = "SELECT GROUP_CONCAT(search_tags_page_rel_group.ID_tag) AS value
                    ...
                    WHERE search_tags_page_rel_group.ID_tag_group = " . $db->toSql($db_gallery->getField("ID", "Number", true), "Number") . "
                        AND search_tags_page_rel_group.ID_tag_page = " . $db->toSql($_REQUEST["keys"]["code"], "Number");
        $db->query($sSQL2); 
        if($db->nextRecord()) {
            ${"value" . $db_gallery->getField("ID", "Number", true)} = $db->getField("value", "Text", true);
        }
        $oField = ffField::factory($cm->oPage);
        $oField->id = "tagGroupRelation_" . $db_gallery->getField("ID", "Number", true);
        $oField->label = ffTemplate::_get_word_by_code("tags_modify_tag_per_type_" . $db_gallery->getField("name", "Text", true));
        $oField->extended_type = "Selection";
        $oField->widget = "autocompletetoken";
        $oField->autocompletetoken_minLength = 0;
        $oField->autocompletetoken_theme = "";
        ...
        $oField->store_in_db = false;
        $oField->default_value = new ffData(${"value" . $db_gallery->getField("ID", "Number", true)}, "Text");
