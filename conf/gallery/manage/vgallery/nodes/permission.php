...
if($ID_vgallery_nodes) {
	$db_gallery->query("SELECT 
	                    ...
	                    WHERE 
	                        vgallery_nodes.ID = " . $db_gallery->tosql($ID_vgallery_nodes, "Number"));
	if($db_gallery->nextRecord()) {
	    $full_path =  $db_gallery->getField("full_path")->getValue();
	    
	    $vgallery_name =  $db_gallery->getField("name")->getValue();
	    $vgallery_nodes_title = ffTemplate::_get_word_by_code("modify_vgallery_nodes") . ": " . $full_path;
	}
...
if($ID_vgallery_nodes) {
    ...
    do {
        ...
        if (strlen($sWhere))
            $sWhere .= " OR ";
        $sWhere .= " (vgallery_nodes.parent = " . $db_gallery->toSql($src_folder_path, "Text") . " AND vgallery_nodes.name = " . $db_gallery->toSql($src_folder_name, "Text") . " )";
    ...
    $sSQL = "SELECT 
                *
                , " . $db_gallery->tosql($ID_vgallery_nodes, "Number") . " AS ID_vgallery_nodes
                , (
                    SELECT
