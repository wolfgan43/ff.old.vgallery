            ...
            if($db_gallery->getField("cascading", "Number", true)) {
                ...
                $sSQL = "SELECT `" . $src["table"] . "`.* 
                        ...
                        WHERE `" . $src["table"] . "`.`" . $src["field"]["parent"] . "` LIKE '" . $db_gallery->tosql($db_gallery->getField("full_path"), "Text", false) . "%' 
                            AND 
                            (
                                `" . $src["table"] . "`.`" . $src["field"]["name"] . "` = " . $db_gallery->toSql(ffCommon_url_rewrite(Auth::get("user")->username)) . "
                            OR    
                                `" . $src["table"] . "`.owner = " . $db_gallery->toSql(Auth::get("user")->id, "Number") . "
                            )
                            "; 
                $db_gallery->query($sSQL);
                if($db_gallery->nextRecord()) {
	...
	if($path == "/" && $vgallery_name_old == $vgallery_name) {
	        ...
	        */ 
	        ...
	        if ($db_gallery->nextRecord()) 
	        {
	            ...
	            do 
	            {
	                ...
	                if($max_upload > 0 && $max_upload < Auth::env("MAX_UPLOAD"))
	    ...
	    */
	    ...
	    if($simple_interface)
	        ...
	        $oRecord->fixed_pre_content = '<h1 class="dialogTitle admin-title vg-content' . ($cm->isXHR() ? "" : " " . Cms::getInstance("frameworkcss")->get(12, "col")) . '">' . Cms::getInstance("frameworkcss")->get("vg-virtual-gallery", "icon-tag", array("2x", "content")) . $vgallery_nodes_title . '</h1>';
...
*/                
       ...
       if(check_function("check_user_request"))
           $request_vgallery = check_user_vgallery_request(array("name" => Auth::get("user")->username, "ID" => Auth::get("user")->id), null, null, null, "rel");
