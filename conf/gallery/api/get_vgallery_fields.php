 ...
 */
function api_get_vgallery_fields($limit_data = null, $params = null, $sort_field = null, $sort_dir = null, $search = null) {
    $db = ffDB_Sql::factory();
    $sort = null;
        
    $schema = array("add_field" => array(
    	"extended_type" => "extended_type.name AS extended_type_name"
    	, "module_type" => "vgallery_type.name AS module_type"
    ));
    ...
    $sSQL = "SELECT vgallery_fields.*
            ...
            WHERE 1 
                ...
                " . (strlen($managed_params["type"])
                    ? " AND vgallery_type.name = " . $db->toSql($managed_params["type"])
                    : ""
                ) . "
		        " . (strlen($limit_data["key"]) && strlen($limit_data["value"]) 
		                ? (strpos($limit_data["value"], ",") === false && !is_numeric($limit_data["value"])
                        	? " AND vgallery_fields.`" . $limit_data["key"] . "` LIKE " . $db->toSql("%" . $limit_data["value"] . "%")
                        	: " AND vgallery_fields.`" . $limit_data["key"] . "` IN (" . $db->toSql($limit_data["value"], "Text", false) . ")"
		                )
		                : "" 
		            ) . "  
            GROUP BY vgallery_fields.ID
