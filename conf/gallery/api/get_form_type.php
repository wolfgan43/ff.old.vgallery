 ...
 */
function api_get_form_type($limit_data = null, $params = null, $sort_field = null, $sort_dir = null, $search = null) {
    $db = ffDB_Sql::factory();
    $sort = null;
            ...
            WHERE 1 
	            ...
	            " . (strlen($limit_data["key"]) && strlen($limit_data["value"]) 
	                    ? (strpos($limit_data["value"], ",") === false && !is_numeric($limit_data["value"])
                        	? " AND module_form.`" . $limit_data["key"] . "` LIKE " . $db->toSql("%" . $limit_data["value"] . "%")
                        	: " AND module_form.`" . $limit_data["key"] . "` IN (" . $db->toSql($limit_data["value"], "Text", false) . ")"
	                    )
	                    : "" 
	                ) . "  
            GROUP BY module_form.ID
