 ...
 */
function api_get_payment($limit_data = null, $params = null, $sort_field = null, $sort_dir = null, $search = null) {
    $db = ffDB_Sql::factory();
	$sort = null;

    $schema = array("add_field" => array(
    	"mpay_name" => "ecommerce_mpay.name AS mpay_name"
    	, "total_payment" => "(ecommerce_documents_payments.value + IF(ecommerce_mpay.enable_vat_calc
    	...
    	, "total_payed" => "ecommerce_documents_payments.payed_value + ecommerce_documents_payments.rebate AS total_payed"
    ));
    ...
    if(strlen($search) || (is_array($sort_field) && count($sort_field)) || (is_array($params) && count($params))) {
        ...
        $sSQL = "SELECT ecommerce_documents_payments.* 
        			$sSQL_add_field_empty
        		FROM ecommerce_documents_payments LIMIT 1";
        $db->query($sSQL);
    ...
    $sSQL = "SELECT ecommerce_documents_payments.*
            ...
            WHERE 1 
                ...
                " . (strlen($managed_params["operation"])
                    ? " AND ecommerce_documents_payments.operation = " . $db->toSql($managed_params["operation"])
                    : ""
                ) . "
		        " . (strlen($limit_data["key"]) && strlen($limit_data["value"]) 
		                ? (strpos($limit_data["value"], ",") === false && !is_numeric($limit_data["value"])
                        	? " AND ecommerce_documents_payments.`" . $limit_data["key"] . "` LIKE " . $db->toSql("%" . $limit_data["value"] . "%")
                        	: " AND ecommerce_documents_payments.`" . $limit_data["key"] . "` IN (" . $db->toSql($limit_data["value"], "Text", false) . ")"
		                )
		                : "" 
		            ) . "  
            ...
            ORDER BY " . ($sort === null 
                ...
                ) . "ecommerce_documents_payments.date DESC";
