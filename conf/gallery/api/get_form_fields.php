 ...
 */
function api_get_form_fields($limit_data = null, $params = null, $sort_field = null, $sort_dir = null, $search = null) {
    $db = ffDB_Sql::factory();
    $sort = null;
        
    $schema = array("add_field" => array(
    	"extended_type" => "extended_type.name AS extended_type_name"
    	, "module_type" => "module_form.name AS module_type"
    ));
	...
	if($params && array_key_exists("type", $params)) {
		$managed_params["type"] = $params["type"];
		unset($params["type"]);
	}
    if(strlen($search) || (is_array($sort_field) && count($sort_field)) || (is_array($params) && count($params))) {
        ...
        $sSQL = "SELECT module_form_fields.* 
        			$sSQL_add_field_empty
        		FROM module_form_fields LIMIT 1";
        $db->query($sSQL);
    ...
    $sSQL = "SELECT module_form_fields.*
            ...
            WHERE 1 
            	$sSQL_Where_params
                " . (strlen($managed_params["type"])
                    ? " AND module_form.name = " . $db->toSql($managed_params["type"])
                    : ""
                ) . "
	            " . (strlen($limit_data["key"]) && strlen($limit_data["value"]) 
	                    ? (strpos($limit_data["value"], ",") === false && !is_numeric($limit_data["value"])
                        	? " AND module_form_fields.`" . $limit_data["key"] . "` LIKE " . $db->toSql("%" . $limit_data["value"] . "%")
                        	: " AND module_form_fields.`" . $limit_data["key"] . "` IN (" . $db->toSql($limit_data["value"], "Text", false) . ")"
	                    )
	                    : "" 
	                ) . "   
            GROUP BY module_form_fields.ID
