 ...
 */
function api_get_form($limit_data = null, $params = null, $sort_field = null, $sort_dir = null, $search = null) {
    $db = ffDB_Sql::factory();
    $schema = array();
    $sort = null;
    ...
    if($ID_form	> 0) {
		...
		$sSQL = "SELECT module_form.ID AS ID
				...
				FROM module_form
					INNER JOIN module_form_nodes ON module_form.ID = module_form_nodes.ID_module
	...
	if(strlen($sSQL)) {
		...
		if($params && array_key_exists("revision", $params)) {
