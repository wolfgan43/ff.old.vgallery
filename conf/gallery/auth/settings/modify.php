...
if($user_path != ffCommon_dirname($user_path)) {
    ...
    do {
        $sSQL_path .= "
            UNION (
                SELECT
                    " . $db_gallery->toSql($tmp_user_path, "Text") . " AS path_value
                    , " . $db_gallery->toSql($tmp_user_path, "Text") . " AS path_name
            ) ";
...
if(!($uid > 0 || $gid > 0)) {
	...
	if($gid > 0) {
		$sSQL = "SELECT name FROM " . CM_TABLE_PREFIX . "mod_security_groups WHERE gid = " . $db_gallery->toSql($gid, "Number");
		$db_gallery->query($sSQL);
		if($db_gallery->nextRecord()) {		
			$tpl->set_var("settings_user_group", "Group: " . $db_gallery->getField("name")->getValue());
			$tpl->parse("SezUserGroupSel", true);
		}
	}
	if($uid > 0) {
		$sSQL = "SELECT username FROM " . CM_TABLE_PREFIX . "mod_security_users WHERE ID = " . $db_gallery->toSql($uid, "Number");
		$db_gallery->query($sSQL);
		if($db_gallery->nextRecord()) {		
			$tpl->set_var("settings_user_group", "User: " . $db_gallery->getField("username")->getValue());
			$tpl->parse("SezUserGroupSel", true);
		}
	}
}
...
$oGrid->source_SQL = "SELECT
                        ...
                        FROM
                            settings
                        WHERE " . (strlen($area) 
                                    ? "settings.area = " . $db_gallery->toSql($area, "Text")
                                    : "1")
                                . "
                        [AND]
                        [WHERE]
...
if($uid > 0 || $gid > 0) {
	...
	$oButton->template_file = "ffButton_image.html";
...
if($groups) {
    $sSQL = "SELECT " . CM_TABLE_PREFIX . "mod_security_groups.gid AS ID_name
            ...
            FROM " . CM_TABLE_PREFIX . "mod_security_groups
                LEFT JOIN settings_rel_path ON settings_rel_path.path =" . $db_gallery->toSql($path, "Text") . "
                    AND settings_rel_path.gid = " . CM_TABLE_PREFIX . "mod_security_groups.gid
            WHERE " . CM_TABLE_PREFIX . "mod_security_groups.gid IN  (" . $db_gallery->toSql($groups, "Text", false) . ")";
    $db_gallery->query($sSQL);
    if($db_gallery->nextRecord()) {
        do {
            $settings_rel_path["groups" . $db_gallery->getField("name")->getValue()]["ID_name"] = $db_gallery->getField("ID_name")->getValue();
            $settings_rel_path["groups" . $db_gallery->getField("name")->getValue()]["name"] = $db_gallery->getField("name")->getValue();
            $settings_rel_path["groups" . $db_gallery->getField("name")->getValue()]["type"] = "Group";
            $settings_rel_path["groups" . $db_gallery->getField("name")->getValue()]["ID_rel_path"] = $db_gallery->getField("ID_rel_path")->getValue();
            $settings_rel_path["groups" . $db_gallery->getField("name")->getValue()]["path"] = $path;
...
if($users) {
    $sSQL = "SELECT 
            ...
            WHERE " . CM_TABLE_PREFIX . "mod_security_users.ID IN  (" . $db_gallery->toSql($users, "Text", false) . ")";
    $db_gallery->query($sSQL);
    if($db_gallery->nextRecord()) {
        do {
            $settings_rel_path["users" . $db_gallery->getField("name")->getValue()]["ID_name"] = $db_gallery->getField("ID_name")->getValue();
            $settings_rel_path["users" . $db_gallery->getField("name")->getValue()]["name"] = $db_gallery->getField("name")->getValue();
            $settings_rel_path["users" . $db_gallery->getField("name")->getValue()]["type"] = "User";
            $settings_rel_path["users" . $db_gallery->getField("name")->getValue()]["ID_rel_path"] = $db_gallery->getField("ID_rel_path")->getValue();
            $settings_rel_path["users" . $db_gallery->getField("name")->getValue()]["path"] = $path;
        } while($db_gallery->nextRecord());
    }
}
...
function settings_on_before_parse_row($component) {
	...
	$db->query($sSQL);
	if($db->nextRecord()) {
		$component->grid_buttons["status"]->image = "ready.png";
	} else {
		$component->grid_buttons["status"]->image = "notready.png";
	}
}
