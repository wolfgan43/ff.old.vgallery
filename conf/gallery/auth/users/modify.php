...
$db_gallery->query("SELECT " . CM_TABLE_PREFIX . "mod_security_users.ID
                        ...
                        WHERE 
                            " . CM_TABLE_PREFIX . "mod_security_users.ID = " . $db_gallery->toSql($uid, "Number"));
...
if($anagraph_type > 0) {
	...
	if($db_gallery->nextRecord()) {
		...
		$default_bill_type 			= $db_gallery->getField("bill_type", "Text", true);

		$default_show_avatar 		= $db_gallery->getField("show_avatar", "Number", true);
		$default_show_avatar_group	= $db_gallery->getField("show_avatar_group", "Number", true);
		$default_avatar_type 		= $db_gallery->getField("avatar_type", "Number", true);
		$default_avatar_model 		= $db_gallery->getField("avatar_model", "Number", true);

		$default_show_custom_group 	= $db_gallery->getField("show_custom_group", "Number", true);
		$default_bill_required 		= $db_gallery->getField("bill_required", "Number", true);
		$default_show_categories	= $db_gallery->getField("show_categories", "Number", true);
		
		$default_show_gmap 			= $db_gallery->getField("show_gmap", "Number", true);
		$default_show_user 			= $db_gallery->getField("show_user", "Number", true);
		$default_show_user_group	= $db_gallery->getField("show_user_group", "Number", true);
		$default_force_user_edit	= $db_gallery->getField("force_user_edit", "Number", true);
		$default_show_vcard			= $db_gallery->getField("show_vcard", "Number", true);
		$default_show_qrcode		= $db_gallery->getField("show_qrcode", "Number", true);
		$default_show_report_group	= $db_gallery->getField("show_report_group", "Number", true);
		$default_use_tab 			= $db_gallery->getField("use_tab", "Number", true);
		$default_force_custom_email	= $db_gallery->getField("force_custom_email", "Number", true);
		$default_force_user_edit	= $db_gallery->getField("force_user_edit", "Number", true);
	}	
}

if(strpos($cm->path_info, VG_SITE_ADMIN) === 0) {
	$is_admin_area = true;
} else {
	$is_admin_area = false;
}
...
if(isset($_REQUEST["frmAction"]) && isset($_REQUEST["setstatus"]) && $uid > 0) {
    ...
    $sSQL = "UPDATE " . CM_TABLE_PREFIX . "mod_security_users 
            SET " . CM_TABLE_PREFIX . "mod_security_users.status = " . $db_gallery->toSql($_REQUEST["setstatus"], "Number") . "
            WHERE " . CM_TABLE_PREFIX . "mod_security_users.ID = " . $db_gallery->toSql($uid, "Number");
		...
		if($anagraph_type > 0 && $count_anagraph_field > 0) {
		    $db_gallery->query("SELECT anagraph_fields.*
									...
									, ( SELECT " . CM_TABLE_PREFIX . "mod_security_users_fields.value 
		                                ...
		                                WHERE " . CM_TABLE_PREFIX . "mod_security_users_fields.ID_users = " . $db_gallery->toSql($uid, "Number") . "
		...
		if($db_gallery->nextRecord()) {
			...
			$db_selection = ffDB_Sql::factory();
		     do {
				...
				$arrField[$field_name]["ID"] = $db_gallery->getField("ID", "Number", true);
				$arrField[$field_name]["name"] = $db_gallery->getField("name")->getValue();
				$arrField[$field_name]["group_field"] = $db_gallery->getField("group_field")->getValue() 
														   ? preg_replace('/[^a-zA-Z0-9]/', '', $db_gallery->getField("group_field")->getValue()) 
														   : ($show_custom_group ? "account" : null);
				$arrField[$field_name]["hide"] = $db_gallery->getField("hide", "Number", true);
				...
				//$arrField[$field_name]["ID_field"] = $db_gallery->getField("ID_field", "Number", true);
				$arrField[$field_name]["ID_selection"] = $db_gallery->getField("ID_selection", "Number", true);
				...
				$arrField[$field_name]["default_value"] = $db_gallery->getField("value")->getValue();
...
if(AREA_USERS_SHOW_SETTINGS && $show_setting_group) {
	...
	if(!$_REQUEST["keys"]["register-ID"]) {
	    $db_gallery->query("SELECT ID FROM " . FF_PREFIX . "languages WHERE " . FF_PREFIX . "languages.status = '1' AND code = " . $db_gallery->toSql(LANGUAGE_INSET, "Text"));
	    if($db_gallery->nextRecord()) {
	...
	$oField->actex_dialog_url = $cm->oPage->site_path . VG_SITE_ADMIN . "/content/modules/register/config/modify";
	$oField->actex_dialog_edit_params = array("keys[registercnf-ID]" => null);
...
if(AREA_GROUPS_SHOW_MODIFY) {
	...
	$strGroups = " AND " . CM_TABLE_PREFIX . "mod_security_groups.level <= (SELECT " . CM_TABLE_PREFIX . "mod_security_groups.level FROM " . CM_TABLE_PREFIX . "mod_security_groups WHERE " . CM_TABLE_PREFIX . "mod_security_groups.gid = " . $db_gallery->toSql($user_permission["primary_gid"], "Number") . ")";
	if($show_user_group) {
		$strGroups = "";
			...
			if(is_array($arrUserGroup) && count($arrUserGroup)) {
				foreach($arrUserGroup AS $arrUserGroup_key => $arrUserGroup_value) {
					if($arrUserGroup_value) {
						...
						$strGroups .= $db_gallery->toSql($arrUserGroup_value);
					}
				}
			}
		} else {
			$strGroups = $db_gallery->toSql($show_user_group);
		}
		if(strlen($strGroups)) {
