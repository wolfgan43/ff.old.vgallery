...
$db_gallery->query("SELECT module_form.*
                        FROM 
                            module_form
                        WHERE 
                            module_form.name = " . $db_gallery->toSql(new ffData($oRecord->user_vars["MD_chk"]["params"][0])));                                                                                                       
if($db_gallery->nextRecord()) {
    ...
    $fixed_pre_content = $db_gallery->getField("fixed_pre_content", "Text", true);  
    $fixed_post_content = $db_gallery->getField("fixed_post_content", "Text", true); 
    ...
    $skip_vat_by_anagraph_type = $db_gallery->getField("skip_vat_by_anagraph_type")->getValue();
...
$db_gallery->query("SELECT module_form_fields.*
                    ...
                    FROM 
                        ...
                        LEFT JOIN check_control ON check_control.ID = " . ($field_default["ID_extended_type"] === null ? " module_form_fields.ID_check_control " : $db_gallery->toSql($field_default["ID_check_control"], "Number")) . " 
...
if($db_gallery->nextRecord()) {
    ...
    setJsRequest("ff.cms.form", "tools");
    ...
    if(!$allow_form) {
       ...
       /* if(strlen($force_from_with_domclass)) {
            ...
            $addit_js = "jQuery('#" . $oRecord->user_vars["MD_chk"]["id"] . "_ffm').val(jQuery('." . $force_from_with_domclass . "').text()); ";
       ...
       // if($_SERVER["HTTP_X_REQUESTED_WITH"] == "XMLHttpRequest" || $oRecord->user_vars["MD_chk"]["ajax"]) {
				...
				$oButton->properties["onclick"] =  "ff.cms.form.insert('" . $oRecord->user_vars["MD_chk"]["id"] . "');";
       ...
       /* } else {
            ...
            $oButton->aspect = "link";
            //$oButton->image = "preview.png";
            $oButton->jsaction = $addit_js . " document.getElementById('frmAction').value = '" . $oRecord->user_vars["MD_chk"]["id"] . "_insert'; jQuery(this).closest('form').submit();"; 
            if($oRecord->user_vars["MD_chk"]["button_label"])
                $oButton->label = $oRecord->user_vars["MD_chk"]["button_label"];
            else
                $oButton->label = ffTemplate::_get_word_by_code(preg_replace('/[^a-zA-Z0-9]/', '', $form_name) . "_insert");
...
//            $oRecord->buttons_options["insert"]["label"] = ffTemplate::_get_word_by_code(preg_replace('/[^a-zA-Z0-9]/', '', $form_name) . "_insert");
		...
		do {
            ...
            if($enable_dep) {
                ...
                $arrDep["source"][$ID_field]["form_name"] = $oRecord->user_vars["MD_chk"]["id"];
            ...
            $field[$ID_field]["form"]["ID"] = $oRecord->user_vars["MD_chk"]["id"];
            ...
            $field[$ID_field]["file_allowed_ext"] = $db_gallery->getField("file_allowed_ext")->getValue();
            ...
            $field[$ID_field]["send_mail"] = ($field_default["send_mail"] === null 
            	? ($db_gallery->getField("send_mail")->getValue() && $db_gallery->getField("check_control")->getValue() == "email" 
            		? true
            		: false
            	)
            	: ($field_default["send_mail"] && $db_gallery->getField("check_control")->getValue() == "email" 
            		? true 
            		: false
            	)
            );
            ...
            $field[$ID_field]["writable"] = ($field_default["writable"] === null ? $db_gallery->getField("writable", "Number", true) : $field_default["writable"]);
            ...
            $field[$ID_field]["placeholder"] = ($field_default["placeholder"] === null ? $db_gallery->getField("placeholder", "Number", true) : $field_default["placeholder"]);
            if(strlen($db_gallery->getField("custom_placeholder", "Text", true)))
                $field[$ID_field]["placeholder"] = ffTemplate::_get_word_by_code($db_gallery->getField("custom_placeholder", "Text", true));
        ...
        if($restore_default_by_cart) {
            ...
            if($db_gallery->nextRecord()) {  
                $oRecord->user_vars["ID_form_node"] =  $db_gallery->getField("ID_form_nodes", "Number", true); 
                do {
            ...
            if(isset($_REQUEST["ph"])) {
                ...
                if(is_array($ph["hc"]) && count($ph["hc"])) {
                    foreach($ph["hc"] AS $ID_form_field => $value) {
                        ...
                        $arrPricelistWhere["hc"][] = " ( module_form_fields.ID = " . $db_gallery->toSql($ID_form_field, "Number") . " AND module_form_pricelist_detail.value = " . $db_gallery->toSql($value) . " ) ";
                    }
                ...
                if(is_array($ph["hca"]) && count($ph["hca"])) {
                    foreach($ph["hca"] AS $ID_form_field => $value) {
                        ...
                        $arrPricelistWhere["hca"][] = " ( module_form_fields.ID = " . $db_gallery->toSql($ID_form_field, "Number") . " AND module_form_pricelist_detail.value = " . $db_gallery->toSql($value) . " ) ";
                    }
                    ...
                    if($db_gallery->getField("check", "Number", true) && array_search($ID_pricelist, $arrPricelistBanned) === false) {
                        if(strlen($db_gallery->getField("value", "Text", true)) && strpos(";", $db_gallery->getField("value", "Text", true)) !== false) {
                            ...
                            foreach($arrTmpValue AS $real_value) {
                				...
                				$arrTmpPricelist[$ID_pricelist . "-" . $arrTmpPricelist[$ID_pricelist]["count_key"]]["fields"]["f-" . $db_gallery->getField("ID_form_fields", "Number", true)] = $real_value;
                				...
                				$arrTmpPricelist[$ID_pricelist]["fields"]["f-" . $db_gallery->getField("ID_form_fields", "Number", true)] = $db_gallery->getField("value", "Text", true);
                            ...
                            if(!strlen($db_gallery->getField("value", "Text", true)))
...
//$obj_page_field->class = ($field_value["ecommerce_class"] == "dynamic-price" && array_search($field_value["ID"], $sum_pfrom) !== false ? "dynamic-pfrom" : $field_value["ecommerce_class"]);
        ...
        if($privacy) {
            ...
            $obj_page_field->default_value = new ffData(ffTemplate::_get_word_by_code("form_privacy_text_" . $oRecord->user_vars["MD_chk"]["params"][0]), "Text");
        ...
        if(strlen($js_form)) {
            ...
            $oPage->tplAddJs($oRecord->id, null, null, false, $oPage->isXHR(), $js_form, false, "bottom");
        }
    }
