...
/*       
            $res = true;
            if(!is_dir(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules/form")) {
                
                $res = @mkdir(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules/form");
                if($res) {
                    $res = @chmod(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules/form", 0777);
                }
            }

            if($res && !is_dir(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path)) {

                $res = @mkdir(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path);
                if($res) {
                    $res = @chmod(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path, 0777);
                }
            }

            if($res) {
                if($component->form_fields["tpl_form_path"]->getValue() == "") {
                    if(@copy(FF_DISK_PATH . FF_THEME_DIR . "/" . cm_getMainTheme() . "/ff/ffRecord/ffRecord.html", FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path . "/form.tpl")) {
                        @chmod(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path . "/form.tpl", 0777);
                        $component->form_fields["tpl_form_path"]->setValue("/modules" . $form_path . "/form.tpl", "Text");
                    }
                }
                if($component->form_fields["tpl_report_path"]->getValue() == "") {
                    if(@copy(FF_DISK_PATH . FF_THEME_DIR . "/" . cm_getMainTheme() . "/ff/ffRecord/ffRecord.html", FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path . "/report.tpl")) {
                        @chmod(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path . "/report.tpl", 0777);
                        $component->form_fields["tpl_report_path"]->setValue("/modules" . $form_path . "/report.tpl", "Text");
                    }
                }
            }
*/
        ...
        case "confirmdelete":
            ...
            //purge_dir(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/modules" . $form_path, $form_path, false);
            break;
...
function MD_form_on_done_action($component, $action) {
    ...
    switch ($action) {
        case "insert":
            ...
            if(Cms::env("AREA_SHOW_ECOMMERCE") && $component->user_vars["enable_ecommerce"]) {
				...
				if(is_array($res) && count($res)) {
	                ...
	                if($component->user_vars["uid"] > 0) {
	                ...
	                } elseif(is_array($component->user_vars["node"])) {
	                    $sSQL = "INSERT INTO comment_rel_module_form 
                    			(
                    				ID
                    				, ID_nodes
                    				, ID_form
                    				...
                    				" . ($component->user_vars["anonymous"] && $component->user_vars["anonymous_show_website"] 
                    					? ", website"
                    					: ""
                    				) . "
                    			)
	                            ...
	                            (
									null
                            		, " . $db->toSql($component->user_vars["node"]["ID"], "Number") . "
                            		, " . $db->toSql($component->key_fields["form-ID"]->default_value, "Number") . "
									, " . $db->toSql($component->user_vars["node"]["tbl_src"], "Text") . "
                            		, " . $db->toSql($component->user_vars["ID_form_node"], "Number") . "
                            		, " . $db->toSql($component->user_vars["node"]["path"], "Text") . "
                            		, " . $db->toSql($user->id, "Number") . "
                    				" . ($component->user_vars["anonymous"]
                    					? "
                            				, " . $db->toSql($component->form_fields["anonymous_nick"]->value) . "
                            				, " . $db->toSql($component->form_fields["anonymous_email"]->value) . "
                    					"
                    					: ""
                    				) . "
									" . ($component->user_vars["anonymous"]  && $component->user_vars["anonymous_show_website"]
										? ", " . $db->toSql($component->form_fields["anonymous_website"]->value) 
										: ""
									) . "
	                            )";
	                    $db->execute($sSQL);
						...
						$sSQL = "UPDATE 
									...
									WHERE 
										layout.ID_type = ( SELECT ID FROM layout_type WHERE layout_type.name = " . $db->toSql("COMMENT") . ")";
						$db->execute($sSQL);
					}
        ...
        case "update": 
        	if(isset($component->form_fields["privacy_check"]) && $component->form_fields["privacy_check"]->getValue() == 0) {
        		$component->displayError(ffTemplate::_get_word_by_code("privacy_check_error"));
        		return false;
			}
				...
				if(is_array($cart_item) && count($cart_item)) {
					foreach($cart_item AS $order_detail) {
						$sSQL = "UPDATE ecommerce_order_detail SET 
									...
									, qta_display = " . $db->toSql($order_detail["custom"]["qta_display"], "Number") . "
								WHERE 
