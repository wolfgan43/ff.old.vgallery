...
if(!isset($_REQUEST["keys"]["swfcnf-ID"])) {
    $db_gallery->query("SELECT module_swf.*
                            FROM 
                                module_swf
                            WHERE 
                                module_swf.name = " . $db_gallery->toSql(new ffData(basename($cm->real_path_info)))
                        );
    if($db_gallery->nextRecord()) {
        $_REQUEST["keys"]["swfcnf-ID"] = $db_gallery->getField("ID", "Number")->getValue();
    } else {
...
{
	$module_swf_title = ffTemplate::_get_word_by_code("modify_module_swf");
	$db_gallery->query("SELECT module_swf.*
                            FROM 
                                module_swf
                            WHERE 
                                module_swf.ID = " . $db_gallery->toSql($_REQUEST["keys"]["swfcnf-ID"], "Number")
                        );
    if($db_gallery->nextRecord()) {
		$module_swf_title .= ": " . $db_gallery->getField("name", "Text", true);
	}
} else
{
	$module_swf_title = ffTemplate::_get_word_by_code("addnew_module_swf");
}
...
$oDetail_fields->populate_insert_SQL = "
                ...
                WHERE 1
                    AND vgallery_type.name <> " . $db_gallery->toSql("System", "Text") . "
