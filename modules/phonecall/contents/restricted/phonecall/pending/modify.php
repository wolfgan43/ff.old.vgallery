...
function PhonecallModify_on_done_action($component, $action) {
    ...
    switch($action) {
		case "insert":
			if(!$component->form_fields["status"]->value->getValue() > 0) {
				$count_to = 0;
				if($component->form_fields["ID_recipient"]->getValue() > 0) {
					$sSQL = "SELECT 
								IF(anagraph.billreference = '', CONCAT(anagraph.name, ' ', anagraph.surname), anagraph.billreference) AS name
								, anagraph.email 
							FROM anagraph 
							WHERE anagraph.ID = " . $db->toSql($component->form_fields["ID_recipient"]->value);
					$db->query($sSQL);
					if($db->nextRecord()) {
						$to[$count_to]["name"] = $db->getField("name", "Text", true);
						$to[$count_to]["mail"] = $db->getField("email", "Text", true);
						$count_to++;
					}
				}
				if($component->form_fields["ID_CC"]->getValue() > 0) {
					$sSQL = "SELECT 
								IF(anagraph.billreference = '', CONCAT(anagraph.name, ' ', anagraph.surname), anagraph.billreference) AS name
								, anagraph.email 
							FROM anagraph 
							WHERE anagraph.ID = " . $db->toSql($component->form_fields["ID_CC"]->value);
					$db->query($sSQL);
					if($db->nextRecord()) {
						$to[$count_to]["name"] = $db->getField("name", "Text", true);
						$to[$count_to]["mail"] = $db->getField("email", "Text", true);
				...
				if(strlen($component->form_fields["email"]->getValue())) {
					$from["name"] = $component->form_fields["reference"]->getValue();  
					$from["mail"] = $component->form_fields["email"]->getValue();  
				} else {
					$from["name"] = $domain_name;
					$from["mail"] = "noreply@" . $domain_name;
				}

				$bcc = null; //$component->user_vars["bcc"];

				$fields = array();
				if(strlen($component->form_fields["date"]->getValue())) {
					$fields["info"]["date"] = $component->form_fields["date"]->getDisplayValue();
				}
				if(strlen($component->form_fields["time"]->getValue())) {
					$fields["info"]["time"] = $component->form_fields["time"]->getDisplayValue();
				}

				if(strlen($component->form_fields["reference"]->getValue())) {
					$fields["applicant"]["reference"] = $component->form_fields["reference"]->getDisplayValue();
				}
				if(strlen($component->form_fields["phone"]->getValue())) {
					$fields["applicant"]["phone"] = $component->form_fields["phone"]->getDisplayValue();
				}
				if(strlen($component->form_fields["fax"]->getValue())) {
					$fields["applicant"]["fax"] = $component->form_fields["fax"]->getDisplayValue();
				}
				if(strlen($component->form_fields["cell"]->getValue())) {
					$fields["applicant"]["cell"] = $component->form_fields["cell"]->getDisplayValue();
				}
				if(strlen($component->form_fields["email"]->getValue())) {
					$fields["applicant"]["email"] = $component->form_fields["email"]->getDisplayValue();
				}

				if(strlen($component->form_fields["message"]->getValue())) {
					$fields["applicant"]["message"] = $component->form_fields["message"]->getDisplayValue();
				}
				
				if(check_function("process_mail")) {
					$res_mail = process_mail(email_system("mod phonecall message", MOD_PHONECALL_THEME), $to, ffTemplate::_get_word_by_code("mod_phonecall_message_subject") . " " . $from["name"], NULL, $fields, $from, $bcc);
				}
			}
			break;
			case "update":
