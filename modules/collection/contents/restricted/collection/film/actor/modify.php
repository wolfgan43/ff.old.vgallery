...
$oRecord->id = "filmActorModify";
$oRecord->resources[] = $oRecord->id;
$oRecord->title = ffTemplate::_get_word_by_code("mod_collection_film_actor_modify_title");
$oRecord->src_table = CM_TABLE_PREFIX . "mod_collection_film_actor";
$oRecord->addEvent("on_done_action", "filmActorModify_on_done_action");
...
function filmActorModify_on_done_action($component, $action) {
    $db = ffDB_Sql::factory();
    ...
    if (strlen($action)) 
    {
        switch ($action) 
		{
			case "insert":
			case "update":
					$sSQL = "UPDATE " . CM_TABLE_PREFIX . "mod_collection_film_actor SET
									" . CM_TABLE_PREFIX . "mod_collection_film_actor.smart_url = " . $db->toSql(ffCommon_url_rewrite($component->form_fields["name"]->getValue())) . "
								WHERE " . CM_TABLE_PREFIX . "mod_collection_film_actor.ID = " . $db->toSql($component->key_fields["ID"]->value, "Number");
					$db->execute($sSQL);
			break;
		}
    }
