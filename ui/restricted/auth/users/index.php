...
if($db->nextRecord())
{
	...
	if(AREA_USER_SHOW_CUSTOM_FIELD) {
		...
		if($db->nextRecord()) {
			...
			$db->query("SELECT DISTINCT anagraph_fields.*
			...
			$db->query("SELECT DISTINCT module_register_fields.*
		    ...
		    do {
		        ...
		        $arrFormField[$key_field]["ID"] .= "'" . $db->getField("name", "Text", true) . "'";
	...
	if($public_area) {
								...
								(
				                    ...
				                    WHERE
				                        (   
				                            ...
				                            WHERE " . CM_TABLE_PREFIX . "mod_security_groups.gid IN (" . $db->toSql($str_ID_groups, "Text", false) . ")
		...
		$oGrid->source_SQL = "
			...
			WHERE " . CM_TABLE_PREFIX . "mod_security_users.ID <> " . $db->toSql(MOD_SEC_GUEST_USER_ID, "Number") . "
			[AND][WHERE]
			[HAVING]
			[ORDER]
		";
