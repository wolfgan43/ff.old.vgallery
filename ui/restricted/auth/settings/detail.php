...
if(isset($_REQUEST["keys"]["ID"])) {
	...
	$sSQL = "SELECT * FROM settings WHERE ID = " . $db->toSql($_REQUEST["keys"]["ID"], "Number");
	$db->query($sSQL);
	if($db->nextRecord()) {
		$criteria = explode(";", $db->getField("criteria")->getValue());
		$value_type = $db->getField("value_type")->getValue();
