...
//da sistemare
//$user_path = str_replace("/" . $UserNID, "", $cm->real_path_info);
$UserID = get_session("UserID");

$ret_url = urldecode($_REQUEST["ret_url"]);
if(!strlen($ret_url)) {
	$ret_url = FF_SITE_PATH . USER_RESTRICTED_PATH;
...
if(strlen(basename($user_path))) {
    $sSQL = "SELECT module_form.name AS form_name, users_rel_module_form.ID_form_node AS ID_form_node
    ...
    if($db->nextRecord()) {
        ...
        ffRedirect(FF_SITE_PATH . USER_RESTRICTED_PATH . "/additionaldata");
    }
}
...
if(!strlen($form_name)) {
    $sSQL = "SELECT " . CM_TABLE_PREFIX . "mod_security_users.ID AS uid
            ...
            ORDER BY users_rel_module_form.request DESC, module_form.`order`, module_form.ID";
    $db->query($sSQL);
    if($db->nextRecord()) {
        $form_name = $db->getField("form_name")->getValue();
        $ID_form_node = $db->getField("ID_form_node")->getValue();
...
$db->query("SELECT module_form_fields.*
                        ...
                        ");
if($db->nextRecord()) {
    $oRecord->id = "UserAdditionaldata";
    $oRecord->class = "UserAdditionaldata";
    ...
    $oRecord->src_table = "";
    $oRecord->title =  ffTemplate::_get_word_by_code("user_additionaldata" . "_" . $form_name . "_title"); 
    ...
    if($additionaldata && $additionaldata != $form_name) {
        $oRecord->ret_url =  USER_RESTRICTED_PATH . "/additionaldata/" . $additionaldata . "?ret_url=" . urlencode($ret_url);
    } else {
        $oRecord->ret_url = $ret_url;
    }
    $oRecord->user_vars["uid"] = $UserNID;
    ...
    $oRecord->buttons_options["insert"]["label"] = ffTemplate::_get_word_by_code("UserAdditionaldata_insert");
    $oRecord->buttons_options["delete"]["label"] = ffTemplate::_get_word_by_code("UserAdditionaldata_delete");
    $oRecord->buttons_options["update"]["label"] = ffTemplate::_get_word_by_code("UserAdditionaldata_update");
    $oRecord->buttons_options["cancel"]["label"] = ffTemplate::_get_word_by_code("UserAdditionaldata_cancel");
	    ...
	    do {
	        ...
	        switch($db->getField("extended_type")->getValue())
	        {
	            ...
	            case "Group":
	                ...
	                $db_selection->query("
	                                    SELECT DISTINCT NULL, nameID, name
	                                    FROM 
	                                    (
	                                        (
	                                            SELECT 
	            ...
	            case "Image":
		            ...
		            $obj_page_field->file_storing_path = DISK_UPDIR . "/users/" . $UserNID . "/" . $form_name;
		            ...
		            $obj_page_field->file_saved_view_url = FF_SITE_PATH . constant("CM_SHOWFILES") . "/users/" . $UserNID . "/" . $form_name . "/[_FILENAME_]";
                    $obj_page_field->file_saved_preview_url = FF_SITE_PATH . constant("CM_SHOWFILES") . "/avatar/users/" . $UserNID . "/" . $form_name . "/[_FILENAME_]";
