...
if($display_account) {
	$db->query("SELECT " . CM_TABLE_PREFIX . "mod_security_users.ID
	                        ...
	                        WHERE 
	                            " . CM_TABLE_PREFIX . "mod_security_users.ID = " . $db->toSql($actual_uid, "Number"));
	if($db->nextRecord()) {
		if($db->getField("public", "Number", true) || $allow_edit) {
		    $_REQUEST["keys"]["register-ID"] = $db->getField("ID")->getValue();
		    $ID_register = $db->getField("ID_register", "Number", true);
	...
	//            $obj_page_field->file_temp_preview_url = FF_SITE_PATH . constant("CM_SHOWFILES") . "/" . $obj_page_field->uploadify_model_thumb . "/[_FILENAME_]";
            ...
            if($anagraph_type > 0 && $count_anagraph_field > 0) {
                $db->query("SELECT anagraph_fields.*
                                        ...
                                        , ( SELECT " . CM_TABLE_PREFIX . "mod_security_users_fields.value 
                                            ...
                                            WHERE " . CM_TABLE_PREFIX . "mod_security_users_fields.ID_users = " . $db->toSql($actual_uid, "Number") . "
	        ...
	        if($db->nextRecord()) {
	            ...
	            do {
					...
					if(check_function("transmute_inlink"))
						$obj_page_field->default_value = new ffData(transmute_inlink($db->getField("value", $type_value)->getValue()), $type_value);
		   ...
		   //additionaldata 
		    ...
		    if($db->nextRecord()) {
		        ...
		        do {
	        		if(!($allow_edit || $db->getField("public", "Number", true)))
	        			continue;
	        		
		            $ID_form = $db->getField("ID_form")->getValue();
