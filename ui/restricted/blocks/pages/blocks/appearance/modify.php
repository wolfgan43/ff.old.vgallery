...
if(!$_REQUEST["extype"] && $cm->real_path_info) {
	...
	$sSQL = "SELECT layout.* 
			...
			FROM layout 
			" . ($ID_layout
				? " WHERE layout.ID = " . $db->toSql($ID_layout, "Number")
				: "
					INNER JOIN layout_path ON layout_path.ID_layout = layout.ID
				WHERE " . $db->toSql($cm->real_path_info) . " LIKE CONCAT(layout_path.ereg_path, IF(layout_path.cascading, '%', ''))
					AND layout.ID_type IN (" . $db->toSql($block_type["virtual-gallery"]["ID"], "Number") . ", " . $db->toSql($block_type["publishing"]["ID"], "Number") . ", " . $db->toSql($block_type["gallery"]["ID"], "Number") . ")
				ORDER BY FIELD(layout.ID_type, " . $db->toSql($block_type["virtual-gallery"]["ID"], "Number") . ", " . $db->toSql($block_type["publishing"]["ID"], "Number") . ", " . $db->toSql($block_type["gallery"]["ID"], "Number") . ")"
			);
	$db->query($sSQL);
	if($db->nextRecord()) {
		$_REQUEST["extype"] = $db->getField("appearance", "Text", true);
		$_REQUEST["path"] = $db->getField("appearance_path", "Text", true);
	}
}
    ...
    if ($db->nextRecord()) {
		...
		if(($item_tbl == "files" || $item_tbl == "vgallery_nodes") && is_numeric($ID_item) && $ID_item > 0 ) {
			$sSQL = "SELECT " . $item_tbl . ".parent
					  ...
					  FROM " . $item_tbl . "
					  WHERE
					  " . $item_tbl . ".ID = " . $db->toSql($ID_item, "Number");
			$db->query($sSQL);
			if($db->nextRecord()) {
...
switch ($item_source_tbl) {
    ...
    case "files":
		...
		$sSQL = "SELECT files.*
        			FROM files
        			WHERE " . ($ID_item > 0 
        					? "files.ID = " . $db->toSql($ID_item, "Number") 
        					: "files.name = " . $db->toSql(basename($item_source_path)
        				) . "
        				AND files.parent = " . $db->toSql(ffCommon_dirname($item_source_path))
			);
		$db->query($sSQL);
...
if(!$sSQL_sort)
	$skip_sort = true;
if(!$sSQL_primary_image)
	$primary_image_simple = true; //http://dev.blueocarina.net/admin/pages/blocks/appearance?&extype=publishing&path=/sdf&&_=1467910689636& da verificare perche rimane blank
   ...
   /* $oRecord->addContent(null, true, "thumb");
	...
	$oRecord->addContent(null, true, $group_thumb_aspect); 
   	$oRecord->groups[$group_thumb_aspect] = array(
										/*"title" => ffTemplate::_get_word_by_code("extras_" . $group_field)
										,*/ "tab" => ($skip_detail && !$allow_fs ? null: $group_thumb)
