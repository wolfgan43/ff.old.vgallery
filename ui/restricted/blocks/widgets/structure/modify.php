...
if(check_function("system_ffcomponent_set_title")) {
	...
	$publishing_name = basename(ffCommon_dirname($cm->real_path_info));
...
, null
, null
, "SELECT publishing_fields.ID
		, IF(" . $src["type"] . "_fields.display_name = ''
			, REPLACE(" . $src["type"] . "_fields.name, '-', ' ')
			, " . $src["type"] . "_fields.display_name
		) AS name
		, publishing_fields.ID_publishing AS ID_publishing
		, publishing_fields.ID_fields AS ID_field
	FROM publishing_fields
		INNER JOIN " . $src["type"] . "_fields ON " . $src["type"] . "_fields.ID = publishing_fields.ID_fields
		INNER JOIN publishing ON publishing.ID = publishing_fields.ID_publishing
	WHERE publishing.name = " . $db->toSql($publishing_name) . "
		AND " . $src["type"] . "_fields.name = " . $db->toSql($field_name)
);
...
$oRecord->insert_additional_fields["ID_publishing"] = new ffData($record["ID_publishing"], "Number");
$oRecord->insert_additional_fields["ID_fields"] = new ffData($record["ID_field"], "Number");
...
//non mette il bottone chiudi con insert new e scegliendo un field
$oRecord->buttons_options["cancel"]["url"] = "[CLOSEDIALOG]";
...
if($record["noentry"]) 
{
    ...
    $oField->source_SQL = "SELECT " . $src["type"] . "_fields.ID
                                    , IF(" . $src["type"] . "_fields.parent_thumb
                                        , CONCAT(" . $src["type"] . "_fields.name,' (', " . $src["type"] . "_fields.parent_thumb, ')')
                                        , " . $src["type"] . "_fields.name
                                    ) AS name
                                    , " . $src["type"] . "_type.name AS grp_name
                                FROM " . $src["type"] . "_fields 
                                    INNER JOIN " . $src["type"] . "_type ON " . $src["type"] . "_type.ID = " . $src["type"] . "_fields.ID_type
                                ORDER BY " . $src["type"] . "_fields.name";
    $oField->widget = "actex";
...
else 
{
   /* if(isset($_REQUEST["keys"]["ID"])) {
    ...
    if(check_function("get_field_default"))
        $arrFieldData = get_field_default($src["type"] . "_fields", $record["ID_field"]);
