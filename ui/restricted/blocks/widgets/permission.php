...
if (!(AREA_PUBLISHING_SHOW_PERMISSION && Cms::env("ENABLE_STD_PERMISSION"))) {
...
if($ID_publishing > 0) {
    ...
    $sSQL = "SELECT 
                *
                , " . $db->tosql($ID_publishing, "Number") . " AS ID_publishing
                , (
                    SELECT
                    ...
                    WHERE 
                        ...
                        AND (
                                (vgallery_nodes.parent =  " . $db->toSql(ffCommon_dirname($full_path), "Text") . " AND vgallery_nodes.name =  " . $db->toSql(basename($full_path), "Text") . ") 
                            OR
                                (
                                    (vgallery_nodes.parent <> " . $db->toSql(ffCommon_dirname($full_path), "Text") . " OR vgallery_nodes.name <>  " . $db->toSql(basename($full_path), "Text") . ") 
                                AND
...
if($type == "node") {
    ...
    $oField->source_SQL = "SELECT DISTINCT 
                            ...
                            HAVING
                            (
                                LENGTH(IF(display_path = '/', display_path, CONCAT(display_path, '/'))) - LENGTH(REPLACE(IF(display_path = '/', display_path, CONCAT(display_path, '/')), '/', '')) < " . $db->toSql($limit_level, "Number") . "
                            )
                            ORDER BY full_path";
}
...
$sSQL_node = "SELECT ID, name 
                ...
                AND name <> 'Directory'
                " . ($limit_type
                        ? " AND vgallery_type.ID IN(" . $db->tosql($limit_type, "Text", false) . ") " 
                        : ""
                    ) ."
                ORDER BY name";
