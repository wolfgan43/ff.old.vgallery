...
if(!isset($_REQUEST["keys"]["ID"]) && isset($_REQUEST["fullpath"]) && strlen($_REQUEST["fullpath"])) {
    ...
    $sSQL = "SELECT `" . $src["table"] . "`.*
            ...
            FROM `" . $src["table"] . "` 
            WHERE `" . $src["table"] . "`.`" . $src["field"]["parent"] . "` = " . $db->toSql(ffCommon_dirname($fullpath)) . "
                AND `" . $src["table"] . "`.`" . $src["field"]["name"] . "` = " . $db->toSql(basename($fullpath)) . "
                " . $src["sql"]["where"]["public"];
    $db->query($sSQL);
    if($db->nextRecord()) {
        $_REQUEST["keys"]["ID"] = $db->getField("ID", "Number", true);
        if(!isset($_REQUEST["vname"])) {
            $_REQUEST["vname"] = $db->getField("vgallery_name", "Text", true);
        }
        if(!isset($_REQUEST["type"])) {
            $_REQUEST["type"] = ($db->getField("is_dir", "Number", true) > 0 ? "dir" : "node");
        }
        if(!isset($_REQUEST["extype"])) {
            $_REQUEST["extype"] = $src["table"];
        }
        if(!isset($_REQUEST["path"])) {
            $_REQUEST["path"] = $db->getField("parent", "Text", true);
        }
...
if (!(AREA_VGALLERY_SHOW_MODIFY)) { 
    ...
    if($db->nextRecord()) {
        ...
        if($db->numRows() > 1) {
            ...
            $limit_parent_path = $db->getField("full_path", "Text", true);
            $vgallery_force_visible = $db->getField("visible", "Number", true);
            
            if($db->getField("cascading", "Number", true)) {
...
if(!strlen($strError)) {
	...
	if($ID_vgallery_nodes > 0)
	{
        ...
        $sSQL = "SELECT `" . $src["table"] . "`.* 
					...
					, " . $src["sql"]["select"]["is_dir"] . " AS is_dir
                FROM `" . $src["table"] . "` 
                WHERE `" . $src["table"] . "`.ID = " . $db->tosql($ID_vgallery_nodes, "Number");
        $db->query($sSQL);
        if($db->nextRecord()) {
            ...
            $vgallery_type              	= $db->getField($src["field"]["ID_type"], "Number", true);
        ...
        $db->query($sSQL);
        if($db->nextRecord()) {
        	$ID_vgallery_nodes_tmp = $db->getField("ID", "Number", true) + 1;
		}
	    ...
	    if($tbl_supp[$src["settings"]]["smart_url"][$vgallery_name]) {
	        ...
	        if($src["settings"]["enable_referer"] && $ID_vgallery_nodes > 0) {
	        	$sSQL = "SELECT vgallery_nodes.ID
	        			FROM vgallery_nodes
	        			WHERE vgallery_nodes.ID_primary_node = " . $db->toSql($ID_vgallery_nodes, "Number");
	        	$db->query($sSQL);
	        	if($db->nextRecord()) {
	        		$vgallery_is_referer = $db->getField("ID", "Number", true);
	        	}
	        }
	    } else {
            $src["settings"] = array();
	...
	switch($src["type"] . "-" . $type) {
		...
		case "anagraph-node":
			$src["sql"]["query"]["parent"] = "	(
											SELECT '/' AS full_path
											, " . $db->toSql(ffTemplate::_get_word_by_code("nothing")) . " AS display_path
										) UNION (
											SELECT DISTINCT 
		                                    ...
		                                    WHERE 1
												" . ($src["settings"]["insert_on_lastlevel"]
		                                            ? " AND LENGTH(CONCAT('/', anagraph_categories.name)) - LENGTH(REPLACE(CONCAT('/', anagraph_categories.name), '/', '')) >= " . $db->tosql($src["settings"]["limit_level"], "Number")
		                                            : ""
		                                        ) . "
		                                        " . (strlen($limit_parent_path)
		                                            ? " AND CONCAT('/', anagraph_categories.name) LIKE '" . $db->toSql($limit_parent_path, "Text", false) . "%'"
		                                            : ""
		                                        ) . "
	        ...
	        $src["sql"]["query"]["ID_type"] = "SELECT " . (($ID_vgallery_nodes > 0 || strlen($default_field_type)) ? "ID" : "name") . "
				                        ...
				                        " . ($src["settings"]["limit_type"]
				                                ? " AND anagraph_type.ID IN(" . $db->tosql($src["settings"]["limit_type"], "Text", false) . ") " 
				                                : ""
				                            ) ."
			                            [ORDER] [COLON] name
			                            [LIMIT]
		...
		case "gallery-node":
			$src["sql"]["query"]["parent"] = "SELECT DISTINCT 
		                                ...
		                                WHERE files.is_dir > 0
											" . ($src["settings"]["insert_on_lastlevel"]
		                                        ? " AND LENGTH(CONCAT(IF(files.parent = '/', '', files.parent), '/', files.name)) - LENGTH(REPLACE(CONCAT(IF(files.parent = '/', '', files.parent), '/', files.name), '/', '')) >= " . $db->tosql($src["settings"]["limit_level"], "Number")
		                                        : ""
		                                    ) . "
		                                    " . (strlen($limit_parent_path)
		                                        ? " AND CONCAT(IF(files.parent = '/', '', files.parent), '/', files.name) LIKE '" . $db->toSql($limit_parent_path, "Text", false) . "%'"
		                                        : ""
		                                    ) . "
		                                [ORDER] [COLON] full_path
		                                [LIMIT]
		...
		case "vgallery-dir":
			$src["sql"]["query"]["parent"] = "SELECT DISTINCT 
	                                        ...
	                                        WHERE vgallery_nodes.is_dir > 0
	                                            ...
	                                            AND vgallery.ID = " . $db->toSql($src["settings"]["ID_vgallery"], "Number") . "
		                                    [ORDER] [COLON] full_path
		                                    [LIMIT]
	                                        ";
	        $src["sql"]["query"]["ID_type"] = "SELECT " . (($ID_vgallery_nodes > 0 || strlen($default_field_type)) ? "ID" : "name") . "
				                        ...
				                        WHERE " . (OLD_VGALLERY 
				                            ...
				                            " . ($src["settings"]["limit_type"]
				                                    ? " AND vgallery_type.ID IN(" . $db->tosql($src["settings"]["limit_type"], "Text", false) . ") " 
				                                    : ""
				                                ) ."
		                                [ORDER] [COLON] NAME
		                                [LIMIT]
		...
		case "vgallery-node":
			$src["sql"]["query"]["parent"] = "SELECT DISTINCT 
	                                    ...
	                                    WHERE vgallery_nodes.is_dir > 0
	                                        ...
	                                        AND vgallery.ID = " . $db->toSql($src["settings"]["ID_vgallery"], "Number") . "
	                                        " . ($src["settings"]["insert_on_lastlevel"]
	                                            ? " AND LENGTH(CONCAT(IF(vgallery_nodes.parent = '/', '', vgallery_nodes.parent), '/', vgallery_nodes.name)) - LENGTH(REPLACE(CONCAT(IF(vgallery_nodes.parent = '/', '', vgallery_nodes.parent), '/', vgallery_nodes.name), '/', '')) >= " . $db->tosql($src["settings"]["limit_level"], "Number")
	                                            : ""
	                                        ) . "
	                                        " . (strlen($limit_parent_path)
	                                            ? " AND CONCAT(IF(vgallery_nodes.parent = '/', '', vgallery_nodes.parent), '/', vgallery_nodes.name) LIKE '" . $db->toSql($limit_parent_path, "Text", false) . "%'"
	                                            : ""
	                                        ) . "
		                                [ORDER] [COLON] full_path
		                                [LIMIT]
		...
		default:
			$src["sql"]["query"]["parent"] = "SELECT DISTINCT 
		                                ...
		                                WHERE `" . $src["table"] . "`.is_dir > 0
											" . ($src["settings"]["insert_on_lastlevel"]
		                                        ? " AND LENGTH(CONCAT(IF(`" . $src["table"] . "`.parent = '/', '', `" . $src["table"] . "`.parent), '/', `" . $src["table"] . "`.name)) - LENGTH(REPLACE(CONCAT(IF(`" . $src["table"] . "`.parent = '/', '', `" . $src["table"] . "`.parent), '/', `" . $src["table"] . "`.name), '/', '')) >= " . $db->tosql($src["settings"]["limit_level"], "Number") 
		                                        : ""
		                                    ) . "
		                                    " . (strlen($limit_parent_path)
		                                        ? " AND CONCAT(IF(`" . $src["table"] . "`.parent = '/', '', `" . $src["table"] . "`.parent), '/', `" . $src["table"] . "`.name) LIKE '" . $db->toSql($limit_parent_path, "Text", false) . "%'"
		                                        : ""
		                                    ) . "
		                                [ORDER] [COLON] full_path
		                                [LIMIT]
		    ...
		    $src["sql"]["query"]["ID_type"] = "SELECT " . (($ID_vgallery_nodes > 0 || strlen($default_field_type)) ? "ID" : "name") . "
	                                    ...
	                                    WHERE 1
	                                    " . ($src["settings"]["limit_type"]
	                                            ? " AND " . $src["type"] . "_type.ID IN(" . $db->tosql($src["settings"]["limit_type"], "Text", false) . ") " 
	                                            : ""
	                                        ) ."
		                                [ORDER] [COLON] name
		                                [LIMIT]
    ...
    */
    ...
    if($src["field"]["public"] && isset($_REQUEST["frmAction"]) && isset($_REQUEST["setpublic"]) && $ID_vgallery_nodes > 0) {
        ...
        $sSQL = "UPDATE `" . $src["table"] . "` 
                SET `" . $src["table"] . "`.`" . $src["field"]["public"] . "` = " . $db->toSql($_REQUEST["setpublic"], "Number") . "
                WHERE `" . $src["table"] . "`.ID = " . $db->toSql($ID_vgallery_nodes, "Number");
        $db->execute($sSQL);
    ...
    if($src["field"]["clone"] && $ID_vgallery_nodes > 0 && $_REQUEST["frmAction"] == "clone") {
        $sSQL = "SELECT `" . $src["table"] . "`.*
                ...
                FROM `" . $src["table"] . "` 
                WHERE `" . $src["table"] . "`.ID = " . $db->toSql($ID_vgallery_nodes);
        $db->query($sSQL);
        if($db->nextRecord()) {
            $params = array(
                ...
                , "vgallery_name" => $db->getField("vgallery_name", "Text", true)
                , "vgallery_parent_old" => $db->getField("parent", "Text", true)
                , "vgallery_name_old" => $db->getField("name", "Text", true)
                , "gallery_model" => ""
            );
        }
        switch($src["type"]) {
		        ...
		        $clone_schema = array(
                    "anagraph" => array(
                            ...
                            , "rel_nodes_dst" => array("real_table" => "rel_nodes", "use_return_ID" => "ID_node_dst", "compare_str" => " 
                                                                    (`rel_nodes`.`ID_node_dst` = " . $db->toSql($ID_vgallery_nodes, "Number") . " 
                            ...
                            , "module_maps_marker" => array("compare_key" => "ID_node", "use_return_ID" => "ID_node", "compare_str" => "
                                                                        `module_maps_marker`.`tbl_src` = " . $db->toSql($src["table"])
                            					)
                        )
                    );
        		break;
		        ...
		        $clone_schema = array(
                    "vgallery" => array(
                            ...
                            , "ecommerce_settings" => array("use_return_ID" => "ID_items", "compare_str" => " 
                                                                    (`ecommerce_settings`.`ID_items` = " . $db->toSql($ID_vgallery_nodes, "Number") . " 
                                                                        AND `ecommerce_settings`.`tbl_src` = " . $db->toSql("vgallery_nodes", "Text") . "
                                                                    )"
	...
	if(AREA_VGALLERY_SHOW_VISIBLE && $ID_vgallery_nodes > 0 && isset($_REQUEST["frmAction"]) && isset($_REQUEST["setvisible"])) {
	    ...
	    $sSQL = "UPDATE " . $src["type"] . "_rel_nodes_fields SET 
	            ...
	            WHERE " . $src["type"] . "_rel_nodes_fields.`nodes` <> ''
	                AND FIND_IN_SET(" . $db->toSql($ID_vgallery_nodes, "Number") . ", " . $src["type"] . "_rel_nodes_fields.`nodes`)";
	    $db->execute($sSQL);
	        ...
	        */
	                ...
	                , "source_SQL" => "SELECT search_tags.code
	                                    ...
	                                    WHERE 
											" . ($src["settings"]["enable_tag"] == -1 
                                                        ? "1"
                                                        : " FIND_IN_SET(" . $db->toSql($src["settings"]["enable_tag"], "Number") . ", search_tags.categories)"
                                                    ) . "	                                    	
			...
			if($ID_vgallery_nodes && !array_key_exists("seo", $page_group)) {
				...
				$sSQL = "SELECT " . $src["seo"]["table"] . ".*
						FROM " . $src["seo"]["table"] . "
						WHERE " . $src["seo"]["table"] . "." . $src["seo"]["rel_key"] . " = " . $db->toSql($ID_vgallery_nodes);
				$db->query($sSQL);
				if($db->nextRecord()) {
					do {
						$ID_lang                                        	= $db->getField($src["seo"]["rel_lang"], "Number", true);
						$lang_code                                      	= $arrLangRev[$ID_lang];
						
						$arrSeo[$lang_code]["permalink"] 					= $db->getField($src["seo"]["permalink"], "Text", true);
						if($src["seo"]["permalink_parent"])
							$arrSeo[$lang_code]["permalink_parent"] 		= $db->getField($src["seo"]["permalink_parent"], "Text", true);
						if($src["seo"]["smart_url"])
							$arrSeo[$lang_code]["smart_url"] 				= $db->getField($src["seo"]["smart_url"], "Text", true);
						if($src["seo"]["title"])
							$arrSeo[$lang_code]["title"] 					= $db->getField($src["seo"]["title"], "Text", true);
						if($src["seo"]["header"])
							$arrSeo[$lang_code]["header"]                   = $db->getField($src["seo"]["header"], "Text", true);
						if($src["seo"]["description"])
							$arrSeo[$lang_code]["description"]              = $db->getField($src["seo"]["description"], "Text", true);
                        
                        if($src["seo"]["robots"])
							$arrSeo[$lang_code]["robots"]                   = $db->getField($src["seo"]["robots"], "Text", true);
                        if($src["seo"]["canonical"])
							$arrSeo[$lang_code]["canonical"]                = $db->getField($src["seo"]["canonical"], "Text", true);
                        if($src["seo"]["meta"])
							$arrSeo[$lang_code]["meta"]                     = $db->getField($src["seo"]["meta"], "Text", true);
                        if($src["seo"]["httpstatus"])
							$arrSeo[$lang_code]["httpstatus"]               = $db->getField($src["seo"]["httpstatus"], "Number", true);
					} while($db->nextRecord());
				}
	        ...
	        */ 
	        $sSQL = " 
	                SELECT " . $src["type"] . "_fields.*
	                    ...
	                    , IF(" . $src["type"] . "_fields.ID_data_type = " .  $db->toSql($tbl_supp["data_type"]["smart_url"]["table.alt"]["ID"], "Number") . "
	                ...
	                WHERE 
	                    " . (strlen($ID_vgallery_nodes) 
	                        ? " (`" . $src["table"] . "`.ID = " . $db->toSql($ID_vgallery_nodes, "Number") . " )"
	                        : " (" . $src["type"] . "_fields.ID_type = " . $db->toSql($default_field_type, "Number") . " )"
	                    ) . "
	                GROUP BY group_data
	        ...
	        if ($db->nextRecord()) 
	        {
	            ...
	            do 
	            {
	                ...
	                $select_data_source 	= $db->getField("selection_data_source", "Text", true);
	                $select_data_limit 		= $db->getField("selection_data_limit", "Text", true);
	                ...
	                $showfiles 				= $tbl_supp["showfiles"]["rev"][$db->getField("settings_type_detail", "Number", true)];
                    ...
                    if($tmp_field["extended_type_group"] == "select") {
                    	if($tmp_field["select"]["data_source"] == "-1") {
                            ...
                            $tmp_field["select"]["sWhere"] = " AND " . $src["type"] . "_rel_nodes_fields.ID_fields = " . $db->toSql($tmp_field["ID_field"], "Number");
                            ...
                            $tmp_field["select"]["sWhere"] = " AND " . $src["type"] . "_fields_selection_value.ID_selection = " . $db->toSql($tmp_field["select"]["data_source"], "Number");
					...
					//$tmp_field["real_name"] = preg_replace('/[^a-zA-Z0-9]/', '', $db->getField("type_name")->getValue() . $field_name);
	                $tmp_field["enable_tip"] 								= $db->getField("enable_tip", "Number", true);
	                ...
	                $tmp_field["user_vars"]["smart_url"] 					= $db->getField("enable_smart_url", "Number", true);
	                $tmp_field["user_vars"]["meta_description"] 			= $db->getField("meta_description", "Text", true);
	                ...
	                $tmp_field["user_vars"]["disable_multilang"] 			= ($tmp_field["data_type"] == "table.alt" || $tmp_field["data_type"] == "relationship" || $tmp_field["data_type"] == "media" || $tmp_field["extended_type_group"] == "select" || $tmp_field["extended_type_group"] == "special"
	                															? true
	                															: $db->getField("disable_multilang", "Number", true)
															                );
	                if($tmp_field["user_vars"]["disable_multilang"] > 0) {
	                    $disable_multilang_count++; 
	                }                

	                $tmp_field["require"] 									= $db->getField("require", "Number", true);
	                $tmp_field["check_control"] 							= $tbl_supp["check_control"]["smart_url"][$check_control]["ff_name"];

					$tmp_field["container_class"]							= $db->getField("field_class_backoffice", "Text", true);

					$field_grid 											= (strlen($db->getField("field_grid_backoffice", "Text", true)) 
																				? explode(",", $db->getField("field_grid_backoffice", "Text", true)) 
																				: ""
																			);					
				    switch($db->getField("field_fluid_backoffice", "Number", true)) {
				        case -1:
					...
					$label_grid 											= (strlen($db->getField("label_grid_backoffice", "Text", true)) 
																				? explode(",", $db->getField("label_grid_backoffice", "Text", true)) 
																				: "" 
																			);

					if($label_grid && array_sum($label_grid) / count($label_grid) < 12) {
					    switch($db->getField("label_fluid_backoffice", "Number", true)) {
					        case 2:
	        			...
	        			if(!$field_rel[$tbl_supp["vgallery"]["smart_url"][$rel_vgallery]["type"]["node"][0]]) {
	        				$page_field_seek = array_search($rel_nodes["keys"][0], array_keys($page_field[LANGUAGE_DEFAULT]));
	        				 $sSQL = " 
					                ...
					                WHERE 
										" . $rel_src["type"] . "_type.name = " . $db->toSql($tbl_supp["vgallery"]["smart_url"][$rel_vgallery]["type"]["node"][0]) . "
										AND " . $rel_src["type"] . "_fields.ID_data_type = " .  $db->toSql($tbl_supp["data_type"]["smart_url"]["relationship"]["ID"], "Number") . "
										AND " . $rel_src["type"] . "_fields.data_source = " . $db->toSql($vgallery_name) . "
					        ...
					        if ($db->nextRecord()) 
					        {
				        		do {
				        			...
				        			$tmp_field["ID_field"] 			= $db->getField("ID", "Text", true); 
				        			$tmp_field["name"] 				= $db->getField("name", "Text", true); 
				        			$tmp_field["smart_url"] 		= ffCommon_url_rewrite($tmp_field["name"]);
				        			$tmp_field["ID"]				= "field_" . ($field_enable_multilang
																		? LANGUAGE_DEFAULT . "_"
																		: ""
																	) . $tmp_field["smart_url"];
				        			//$tmp_field["data_source"] 	= $db->getField("data_source", "Text", true); 
				        			//$tmp_field["data_limit"] 		= $db->getField("data_limit", "Text", true); 
	                            ...
	                            switch($field_value["data_type"]) 
	                            {
										...
										if($ID_vgallery_nodes > 0) {
	                            			if(OLD_VGALLERY) {
												$sSQL_field .= ", (
			                                        ...
			                                        WHERE ID_nodes = " . $db->toSql($ID_vgallery_nodes, "Number") . "
														" . ($src["seo"]["rel_field"]
			                                                    ? " AND `" . $src["seo"]["rel_field"] . "` = " . $db->toSql($field_value["ID_field"], "Number")
			                                                    : ""
			                                            ) . "
			                                            " . ($src["seo"]["rel_lang"]
			                                                    ? " AND `" . $src["seo"]["rel_lang"] . "` = " . $db->toSql($field_value["user_vars"]["ID_lang"], "Number")
			                                                    : ""
			                                            ) . "
			                                        LIMIT 1
			                                    ) AS `" . $field_key . "`";				                            		
												...
												$sSQL_field .= ", (
			                                        ...
			                                        FROM " . $src["seo"]["table"] . "
			                                        WHERE " . $src["seo"]["rel_key"] . " = " . $db->toSql($ID_vgallery_nodes, "Number") . "
														" . ($src["seo"]["rel_field"]
			                                                    ? " AND `" . $src["seo"]["rel_field"] . "` = " . $db->toSql($field_value["ID_field"], "Number")
			                                                    : ""
			                                            ) . "		                                            
	                            		...
	                            		if($ID_vgallery_nodes > 0) {
	                            			if(is_array($field_value["user_vars"]["src"])) {
												$sSQL_field .= ", (
							                                    ...
							                                    WHERE FIND_IN_SET(" . $db->toSql($ID_vgallery_nodes, "Number") . ", `limit`)
							                                    	AND " . $src["type"] . ".name = " . $db->toSql($field_value["data_source"]) . "
							                                        AND ID_fields = " . $db->toSql($field_value["ID_field"], "Number") . "
							                                        " . ($src["field"]["lang"]
							                                                ? " AND `" . $src["field"]["lang"] . "` = " . $db->toSql($field_value["user_vars"]["ID_lang"], "Number")
							                                                : ""
							                                        ) . "
							                                ) AS `" . $field_key . "`";				                            			
			                                            ...
			                                            INNER JOIN rel_nodes
			                                                ...
			                                                (
			                                                    (
			                                                        ...
			                                                        AND rel_nodes.contest_src = " . $db->toSql($field_value["data_source"], "Text") . "
			                                                        AND rel_nodes.contest_dst = " . $db->toSql($vgallery_name, "Text") . " 
			                                                        AND rel_nodes.ID_node_dst = " . $db->toSql($ID_vgallery_nodes, "Number") . " 
			                                                    ) 
			                                                OR 
			                                                    (
			                                                        rel_nodes.ID_node_dst = `" . $field_value["data_source_table"] . "`.ID
			                                                        AND rel_nodes.contest_dst = " . $db->toSql($field_value["data_source"], "Text") . "
			                                                        AND rel_nodes.contest_src = " . $db->toSql($vgallery_name, "Text") . " 
			                                                        AND rel_nodes.ID_node_src = " . $db->toSql($ID_vgallery_nodes, "Number") . " 
			                                                    )
			                                                )
			                                            WHERE 1
			                                    ) AS `" . $field_key . "`";
	                            				...
	                            				foreach($schema["db"]["data_source"][$field_value["data_source"]]["fields"] AS $schema_field_key => $schema_field_data) {
													if($schema_field_data["hide"] && $schema_field_data["require"]) {
														...
														if(strlen($schema_field_value))
															$schema["db"]["data_source"][$field_value["data_source"]]["sWhere"] .= " AND `" . $field_value["data_source"] . "`.`" . $schema_field_key . "` = " . $db->toSql($schema_field_value);
													}
	                            				}
	                            			}
											...
											if(count($arrDataLimit) > 1) {
												...
												if($field_value["extended_type"] == "GMap") 
												{
	                                    			$sSQL_field .= ", (
				                                                ...
				                                                WHERE `" . $field_value["data_source"] . "`.`" . $key . "` = " . $db->toSql($ID_vgallery_nodes, "Number") . "
	                                                        		" . $schema["db"]["data_source"][$field_value["data_source"]]["sWhere"] . "
				                                                    " . ($src["field"]["lang"]
	                                                            		? " AND `" . $field_value["data_source"] . "`.`" . $src["field"]["lang"] . "` = " . $db->toSql($field_value["user_vars"]["ID_lang"], "Number")
	                                                            		: ""
				                                                    ) . "
				                                                LIMIT 1
				                                                ) AS `" . $field_key . "_lat`";														
		                                            ...
		                                            if($field_value["data_source"] == $src["table"]) {
	                            						...
	                            						$sSQL_field .= ", ( SELECT `" .  $field_value["data_limit"] . "`
	                            							FROM `" . $field_value["data_source"] . "`
	                            							WHERE `" . $key . "` = " . $db->toSql($ID_vgallery_nodes, "Number") . "
	                            								" . $schema["db"]["data_source"][$field_value["data_source"]]["sWhere"] . "
	                            							LIMIT 1
	                            						) AS `" . $field_key . "`"; 
		                                            }
	                            		...
	                            		if(is_array($schema["db"]["data_source"][$tbl_map_marker]["record_params"]) && count($schema["db"]["data_source"][$tbl_map_marker]["record_params"])) {
	                            			foreach($schema["db"]["data_source"][$tbl_map_marker]["record_params"] AS $record_param_key => $record_param_value) {
	                            				if($record_param_value === null) {
	                            					$component["record_params"] .= $record_param_key . "=" . $db->toSql(
	                            						($ID_vgallery_nodes
								    						? $ID_vgallery_nodes
								    						: $ID_vgallery_nodes_tmp
								    					), "Number", false) . "&";
												} elseif(strlen($record_param_value) && is_string($record_param_value)) {
	                            					$component["record_params"] .= $record_param_key . "=" . $db->toSql($record_param_value, "Text", false) . "&";
	                            					$component["where"][] = " AND `" . $record_param_key . "`=" .  $db->toSql($record_param_value);
													...
													if($record_param_value["request"] && $_REQUEST[$record_param_value["request"]]) {
	                            						$component["record_params"] .= $record_param_value["request"] . "=" . $db->toSql($_REQUEST[$record_param_value["request"]], "Text", false) . "&";
	                            						...
	                            						if($record_param_value["field"])
	                            							$component["where"][] = " AND `" . $record_param_value["field"] . "`=" .  $db->toSql($_REQUEST[$record_param_value["request"]]);
														
													} else {
	                            						$component["record_params"] .= $record_param_key . "=" . $db->toSql($_REQUEST[$record_param_key], "Text", false) . "&";
	                            						$component["where"][] = " AND `" . $record_param_key . "`=" .  $db->toSql($_REQUEST[$record_param_key]);
									...
									case "media":
										if($ID_vgallery_nodes > 0) {
											...
											$sSQL_field .= ", (" . $db->toSql((is_array($arrGallery) 
												? implode(",", $arrGallery)
												: ""
											)) . ") AS `" . $field_key . "`";	
	/*
										...
										if($ID_vgallery_nodes > 0) {
											if($field_value["extended_type"] == "GMap") 
											{
												$sSQL_field .= ", (
		                                                    ...
		                                                    WHERE `module_maps_marker`.`ID_node` = " . $db->toSql($ID_vgallery_nodes, "Number") . "
		                                                        AND `module_maps_marker`.`tbl_src` = " . $db->toSql($src["table"]) . "
																" . ($src["field"]["lang"]
																	? " AND `module_maps_marker`.`" . $src["field"]["lang"] . "` = " . $db->toSql($field_value["user_vars"]["ID_lang"], "Number")
																	: ""
																) . "
		                                                    LIMIT 1
		                                                    ) AS `" . $field_key . "_lat`"; 
	                            ...
	                            if(!$component["name"] && !$ID_vgallery_nodes) {
		                            $sSQL_field .= ", (
		                                                " . $db->toSql($field_value["default_value"], $field_value["ff_extended_type"]). "
		                                                ) AS `" . $field_key . "`"; 
							...
							switch($component["name"]) {
								case "grid":
								    ...
								    $obj_page_field->source_SQL = "SELECT `" . $component["data_source"] . "`.* 
								    							...
								    							WHERE `" . $component["data_source"] . "`.`" . $component["compare_key"] . "` = " . $db->toSql(
								    								($ID_vgallery_nodes
								    									? $ID_vgallery_nodes
								    									: $ID_vgallery_nodes_tmp
								    								), "Number") 
	            ...
	            if(strlen($sSQL_field)) {
	                ...
	                $oRecord->populate_insert_SQL = "SELECT 
	                                                    " . $db->toSql($path) . " AS `parent`
	                                                    , " . $db->toSql($type == "node" ? "0" : "1", "Number") . " AS `id_dir`
	                                                    , " . $db->toSql("load fadeIn") . " AS `ajax_on_event`
	                                                    , " . $db->toSql("0", "Number") . " AS `highlight`
	                                                    , " . $db->toSql("1", "Number") . " AS `visible`
	                                                    , " . $db->toSql("") . " AS `tags`
	                                                    , " . $db->toSql("") . " AS `place_title`
	                                                    , " . $db->toSql("") . " AS `place_lat`
	                                                    , " . $db->toSql("") . " AS `place_lng`
	                                                    , " . $db->toSql("") . " AS `place_zoom`
	                                                    , " . $db->toSql("") . " AS `ID_place`
	                                                    $sSQL_field
	                                                ";
		...
		$cm->oPage->tplAddJs("ff.cms.admin.vgallery-nodes-modify", array(
			"embed" => $js
		));  	        
