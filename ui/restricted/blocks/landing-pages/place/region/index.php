...
if(isset($_REQUEST["repair"])) {
    ...
    if($db->nextRecord()) {
        ...
        do {
            ...
            if($address) {
                set_region_by_address_info($address);            
            }
        } while($db->nextRecord());
    }
}
...
if($_REQUEST["ID_state"])
	$arrWhere[] = FF_SUPPORT_PREFIX . "region.ID_state = " . $db->toSql($_REQUEST["ID_state"], "Number", true);
...
$oButton->url = $cm->oPage->site_path . ffCommon_dirname($cm->oPage->page_path) . "/city?ID_region=[ID_VALUE]";
$oButton->aspect = "link";
...
$oButton->id = "province";
$oButton->action_type = "gotourl";
$oButton->url = $cm->oPage->site_path . ffCommon_dirname($cm->oPage->page_path) . "/province?ID_region=[ID_VALUE]";
$oButton->aspect = "link";
