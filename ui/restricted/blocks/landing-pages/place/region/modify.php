...
$res = system_ffComponent_resolve_record(FF_SUPPORT_PREFIX . "region");

// -------------------------
//          RECORD
// -------------------------
$oRecord = ffRecord::factory($cm->oPage);
$oRecord->id = "PlaceRegionModify";
$oRecord->resources[] = $oRecord->id;
$oRecord->src_table = FF_SUPPORT_PREFIX . "region";
$oRecord->tab = true;
$oRecord->addEvent("on_do_action", "PlaceRegionModify_on_do_action");
$oRecord->addEvent("on_done_action", "PlaceRegionModify_on_done_action");
...
$oField = ffField::factory($cm->oPage);
$oField->id = "name";
$oField->label = ffTemplate::_get_word_by_code("place_region_name");
...
$oField = ffField::factory($cm->oPage);
$oField->id = "zone";
$oField->label = ffTemplate::_get_word_by_code("place_region_zone");
$oField->setWidthComponent(2);
$oRecord->addContent($oField, $group_field);

$oField = ffField::factory($cm->oPage);
$oField->id = "smart_url";
$oField->label = ffTemplate::_get_word_by_code("place_region_smart_url");
$oField->widget = "slug";
...
$oField = ffField::factory($cm->oPage);
$oField->id = "coord";
$oField->label = ffTemplate::_get_word_by_code("place_region_coord");
$oField->widget = "gmap";
...
$oField = ffField::factory($cm->oPage);
$oField->id = "visible";
$oField->label = ffTemplate::_get_word_by_code("place_region_visible");
$oField->base_type = "Number";
...
function PlaceRegionModify_on_do_action($component, $action) {
    $db = ffDB_Sql::factory();
	
	switch($action) {
        case "insert":
            $sSQL = "SELECT " . FF_SUPPORT_PREFIX . "region.*
                    FROM " . FF_SUPPORT_PREFIX . "region
                    WHERE " . FF_SUPPORT_PREFIX . "region.smart_url = " . $db->toSql($component->form_fields["smart_url"]->value);
            $db->query($sSQL);
            if($db->nextRecord()) {
                $component->tplDisplayError(ffTemplate::_get_word_by_code("duplicate_region_smart_url"));
                return true;
            }
            break;
        case "update":
            $sSQL = "SELECT " . FF_SUPPORT_PREFIX . "region.*
                    FROM " . FF_SUPPORT_PREFIX . "region
                    WHERE " . FF_SUPPORT_PREFIX . "region.smart_url = " . $db->toSql($component->form_fields["smart_url"]->value) . "
                        AND " . FF_SUPPORT_PREFIX . "region.ID <> " . $db->toSql($component->key_fields["ID"]->value);
            $db->query($sSQL);
            if($db->nextRecord()) {
                $ID_region = $db->getField("ID", "Number", true);

                $sSQL = "UPDATE " . FF_SUPPORT_PREFIX . "province SET ID_region = " . $db->toSql($ID_region, "Number") . "
                        WHERE " . FF_SUPPORT_PREFIX . "province.ID_region = " . $db->toSql($component->key_fields["ID"]->value);
                $db->execute($sSQL);
                $sSQL = "UPDATE " . FF_SUPPORT_PREFIX . "city SET ID_region = " . $db->toSql($ID_region, "Number") . "
                        WHERE " . FF_SUPPORT_PREFIX . "city.ID_region = " . $db->toSql($component->key_fields["ID"]->value);
                $db->execute($sSQL);
...
function PlaceRegionModify_on_done_action($component, $action) {
	$db = ffDB_Sql::factory();
	...
	switch($action) {
		...
		case "update":
            ...
            set_region_by_address_info(array(
