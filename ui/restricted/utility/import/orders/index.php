<?php
    ...
    if($frmAction == "scan") {
                    ...
                    while (($data_csv = fgetcsv($fp, 0, $sep, $enclosure)) !== FALSE) {
                        ...
                        if(is_array($data_csv) && count($data_csv)) {
                            ...
                            if(is_array($structure) && count($structure)) {
                                ...
                                $order_id = get_structure_data($data_csv, $structure["order_id"], "-", $db);
                                ...
                                $shipping = get_structure_data($data_csv, $structure["shipping"], "-", $db);
								...
								} elseif(strlen($shipping)) {
	                                $sSQL = "SELECT * FROM ecommerce_shipping WHERE name LIKE '%" . $db->toSql($shipping, "Text", false) . "%'";
	                                $db->query($sSQL);
									if($db->nextRecord()) {
										$ID_shipping = $db->getField("ID", "Number")->getValue();
									} else {
	                                    $sSQL = "INSERT INTO `ecommerce_shipping` 
	                                            (
	                                                `ID`
	                                                , `name`
	                                                , `status`
	                                    ...
	                                    $db->execute($sSQL);
	                                    $ID_shipping = $db->getInsertID(true);
									}
								} else {
									$ID_shipping = 0;
								}	
								
								if(strlen($bill_total) && $bill_total > 0) {
	                                ...
	                                $sSQL = "SELECT * FROM ecommerce_documents_bill WHERE bill_id = " . $db->toSql($order_id, "Number") . " AND date = " . $db->toSql($date, "Date");
	                                $db->query($sSQL);
	                                if($db->nextRecord()) {
	                                    ...
	                                    $sSQL = "UPDATE `ecommerce_documents_bill` 
	                                            SET 
	                                                ...
	                                                , `operation` = " . $db->toSql("sent") . " 
	                                                , `ID_type` = " . $db->toSql($ID_type, "Number") . " 
	                                                , `pdf` = " . $db->toSql("") . " 
	                                                , `bill_id` = " . $db->toSql($order_id, "Number") . " 
                                ...
                                $sSQL = "SELECT * FROM ecommerce_order WHERE SID = " . $db->toSql($primary_key);
                                $db->query($sSQL);
                                if($db->nextRecord()) {
                                    $ID_record = $db->getField("ID", "Number")->getValue();

                                    $sSQL = "UPDATE ecommerce_order
                                            SET 
                                            	`ID_type` = " . $db->toSql($ID_type, "Number") . " 
                                                , `order_id` = " . $db->toSql($order_id, "Number") . " 
                                                , `ID_anagraph` = " . $db->toSql($ID_anagraph, "Number") . " 
                                                , `last_update` = " . $db->toSql($last_update) . " 
                                                , `date` = " . $db->toSql($date, "Date") . " 
                                                , `object` = " . $db->toSql($object) . " 
                                                ...
                                                , `ID_ecommerce_shipping` = " . $db->toSql($ID_shipping, "Number") . "
                                            WHERE `ecommerce_order`.`ID` = " . $db->toSql($ID_record, "Number");
                                    $db->execute($sSQL);
                                    if($db->affectedRows())
                                        $count_rec_update++;
                                        
                                } else {
                                    $sSQL = "SELECT * FROM ecommerce_order WHERE ID = " . $db->toSql($primary_key);
                                    $db->query($sSQL);
                                    if($db->nextRecord()) {
                                        $new_key = "NULL";
                                    } else {
                                    ...
                                    $sSQL = "INSERT INTO ecommerce_order
                                            (
                                                `ID`
                                                , `SID`
                                                , `ID_type` 
                                                ...
                                                , `owner`
                                                , `ID_bill`
                                                , `ID_ecommerce_shipping`
                                            )
                                            VALUES 
                                            (
                                                $new_key 
                                                , " . $db->toSql($primary_key, "Number") . "
                                                , " . $db->toSql($ID_type, "Number") . " 
                                                , " . $db->toSql($order_id, "Number") . "
                                                , " . $db->toSql($ID_anagraph, "Number") . "
							                    , " . $db->toSql($last_update) . "
        ...
        if(strlen($del_order)) {
            $sSQL = "DELETE FROM ecommerce_documents_bill WHERE ecommerce_documents_bill.ID IN ( SELECT ecommerce_order.ID_bill FROM ecommerce_order WHERE  ecommerce_order.last_update = " . $db->toSql($del_order) . " AND ecommerce_order.owner = '-1') ";
            $db->execute($sSQL);

            $sSQL = "DELETE FROM ecommerce_order WHERE ecommerce_order.last_update = " . $db->toSql($del_order) . " AND ecommerce_order.owner = '-1'";
            $db->execute($sSQL);
    ...
    $oField->source_SQL = "SELECT DISTINCT
                                ecommerce_order.last_update
                                ,  CONCAT('(', (SELECT COUNT(*) FROM ecommerce_order AS tbl WHERE tbl.last_update = ecommerce_order.last_update), ') ',  FROM_UNIXTIME(ecommerce_order.last_update)) AS value
                           FROM
                                ecommerce_order
                           WHERE ecommerce_order.owner = '-1'
                          ";   
