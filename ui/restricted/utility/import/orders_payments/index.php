<?php
    ...
    if($frmAction == "scan") {
        ...
        if($enclosure && $destination_type && $field_type && $nrec) {
            if(is_file(FF_DISK_PATH . FF_THEME_DIR . "/" . FRONTEND_THEME . "/" . GALLERY_TPL_PATH . "/import" . $file_scan)) {
                ...
                if(isset($structure["primary"])) {
                    ...
                    while (($data_csv = fgetcsv($fp, 0, $sep, $enclosure)) !== FALSE) {
                        ...
                        if(is_array($data_csv) && count($data_csv)) {
                            ...
                            if(is_array($structure) && count($structure)) {
								...
								$sSQL = "SELECT * FROM ecommerce_order WHERE SID = " . $db->toSql($order_id);
	                            $db->query($sSQL);
	                            if($db->nextRecord()) {
	                                $ID_order = $db->getField("ID", "Number")->getValue();
									$ID_bill = $db->getField("ID_bill", "Number")->getValue();

									$fill_order[$ID_bill]["order_id"] = $ID_order;
									$fill_order[$ID_bill]["value"] = $fill_order[$ID_bill]["value"] + $payments_total;
									$fill_order[$ID_bill]["payed_value"] = $fill_order[$ID_bill]["payed_value"] + $payed_value;
	                                ...
	                                if($ID_bill > 0 && $ID_mpay > 0) {
		                                $sSQL = "SELECT * 
		                                		...
		                                		WHERE ecommerce_documents_payments.status > 0 
		                                			...
		                                			AND ecommerce_documents_payments.SID = " . $db->toSql($primary_key);
		                                $db->query($sSQL);
		                                if($db->nextRecord()) {
                    ...
                    if($visible) {
                    	if(is_array($fill_order) && count($fill_order)) {
                    		foreach($fill_order AS $fill_order_key => $fill_order_value) {
                    			if(check_function("ecommerce_get_total_by_order"))
                    				$total_order = ecommerce_get_total_by_order($fill_order_value["order_id"]);
                    			if($total_order["total_order"] > $fill_order_value["value"]) {
                    				$total_discount = ($total_order["total_order"] - $fill_order_value["value"]);
	                                $sSQL = "UPDATE `ecommerce_documents_bill` 
	                                        ...
	                                        WHERE `ecommerce_documents_bill`.`ID` = " . $db->toSql($fill_order_key, "Number");
	                                $db->execute($sSQL);
								}
							}
						}
        ...
        if(strlen($del_order)) {
            $sSQL = "DELETE FROM ecommerce_documents_payments WHERE ecommerce_documents_payments.last_update = " . $db->toSql($del_order) . " AND ecommerce_documents_payments.owner = '-1'";
            $db->execute($sSQL);
        }
    ...
    $oField->source_SQL = "SELECT
                                'order_payments' AS ID
                                , 'order_payments' AS name
                          ";   
