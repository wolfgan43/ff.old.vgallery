	...
	if($_REQUEST["importcsv"] == "continue") {
		...
		$js = '
			function continueImportCSV(count, page) {
				...
				jQuery.getJSON("' . FF_SITE_PATH . $cm->oPage->page_path . "/csv?" . (isset($_REQUEST["XHR_CTX_ID"]) ? "XHR_CTX_ID=" . $_REQUEST["XHR_CTX_ID"] . "&" : "") . "importcsv=continue" . (isset($_REQUEST["ret_url"]) ? "&ret_url=" . rawurlencode($_REQUEST["ret_url"]) : "") . '", function(data) { 
					if(data.callback)
						eval(data.callback);
				}); 
			};';
