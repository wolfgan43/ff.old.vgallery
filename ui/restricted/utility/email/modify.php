...
if($_REQUEST["keys"]["email-ID"] > 0) {
	$sSQL = "SELECT email.* FROM email WHERE email.ID = " . $db->toSql($_REQUEST["keys"]["email-ID"], "Number");
	$db->query($sSQL);
	if($db->nextRecord()) {
		$email_name = $db->getField("name", "Text", true);
	}
}
...
if(is_array($tpl_email) && count($tpl_email)) {
	foreach($tpl_email AS $real_file) {
		if(is_dir($real_file) && is_file($real_file . "/email.tpl")) {
			...
			$sSQL_part .= "(
		            		SELECT " . $db->toSql($tpl_email_path) . " AS ID
		            			, " . $db->toSql($tpl_email_path) . " AS name
		            		)";
