...
$record = system_ffComponent_resolve_record("module_form"
		, array(
			"enable_ecommerce_weight" => null
		)
);

$module_form_title = ffTemplate::_get_word_by_code("modify_multiple_module_form_pricelist");
...
if(isset($_REQUEST["list-ID"]) && strlen($_REQUEST["list-ID"]))
{
    ...
    $sSQL_Where = " AND module_form_pricelist.ID_module = " . $db->toSql($_REQUEST["keys"]["ID"], "Number");
}
...
$oRecord->user_vars["ID"] = $_REQUEST["keys"]["ID"];
/* Title Block */
...
} else 
{
    ...
    $oField->source_SQL = "SELECT DISTINCT module_form_pricelist.price,
                                ...
                                WHERE module_form_pricelist.ID_module = " . $db->toSql($_REQUEST["keys"]["ID"], "Number") . "[AND] [WHERE]
                            [HAVING]
                                ORDER BY price";
    $oField->actex_update_from_db = true;
    ...
    if(is_array($arrPricelist) && count($arrPricelist)) 
    {
        if(check_function("get_field_by_extension"))
        {
            ...
            {
                $oField = ffField::factory($cm->oPage);
                get_field_by_extension($oField, $arrPricelist_value["field"], "form");
                //$oField = $field_ext["obj"];
    ...
    if($record["enable_ecommerce_weight"])
    {
        $oField = ffField::factory($cm->oPage);
...
function FormExtraMultipleModifySelectedPricelist_on_done_action($component, $action) {
    ...
    if(strlen($action))
    {
        switch ($action) {
            case "insert":
            case "update":
                if(isset($component->user_vars["ID"]) && $component->user_vars["ID"] > 0)
                {
                    if($component->form_fields["field_checkbox"]->getValue())
                    {
                        ...
                        if(strlen($sSQL_select))
                        {
                            $sSQL = "SELECT module_form_pricelist_detail.ID
                                        ...
                                        WHERE module_form_pricelist.ID_module = " . $db->toSql($component->user_vars["ID"], "Number") . "
                                ...
                                if(strlen($sSQL_pricelist_where)) {
                                    $sSQL = "SELECT module_form_pricelist.ID, COUNT(module_form_pricelist.ID) AS count_id
                                               ...
                                               WHERE (" . $sSQL_pricelist_where . ")
                                                    AND module_form_pricelist.ID_module = " . $db->toSql($component->user_vars["ID"], "Number") . "
