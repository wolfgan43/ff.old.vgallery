        ...
        switch ($action) {
                ...
                if (!$component->user_vars["disable_account_registration"]) {
                    if(isset($component->form_fields["username"]))
                    {
                        if (strlen($component->form_fields["username"]->getValue())) {
                            $sSQL = "SELECT ID FROM `" . $component->src_table . "` WHERE username = " . $db->toSql($component->form_fields["username"]->value);
                            $db->query($sSQL);
                            if ($db->numRows() > 0) {
                    ...
                    if (strlen($component->form_fields["email"]->getValue())) {
                        $sSQL = "SELECT ID FROM `" . $component->src_table . "` WHERE email = " . $db->toSql($component->form_fields["email"]->value);
                        $db->query($sSQL);
                        if ($db->numRows() > 0) {
            ...
            case "update":
                if (!$component->user_vars["disable_account_registration"]) {
                    ...
                    {
                        if (strlen($component->form_fields["username"]->getValue())) {
                            $sSQL = "SELECT ID FROM `" . $component->src_table . "` WHERE username = " . $db->toSql($component->form_fields["username"]->value) . " AND ID <> " . $db->toSql($component->key_fields["register-ID"]->value);
                            $db->query($sSQL);
                            if ($db->numRows() > 0) {
                                return ffTemplate::_get_word_by_code("username_not_unic_value");
                            }
                    ...
                    if (strlen($component->form_fields["email"]->getValue())) {
                        $sSQL = "SELECT ID FROM `" . $component->src_table . "` WHERE email = " . $db->toSql($component->form_fields["email"]->value) . " AND ID <> " . $db->toSql($component->key_fields["register-ID"]->value);
                        $db->query($sSQL);
                        if ($db->numRows() > 0) {
                            return ffTemplate::_get_word_by_code("email_not_unic_value");
                        }
