...
$db->query("SELECT " . CM_TABLE_PREFIX . "mod_security_users.ID AS uid
...
if($db->nextRecord()) {
    ...
    if($db->nextRecord()) {
        ...
        do {
            ...
            $obj_page_field->user_vars["enable_in_mail"] = $db->getField("enable_in_mail")->getValue();
            ...
            switch($db->getField("extended_type")->getValue())
            {
                ...
                case "Text":
                    ...
                    if(!$writable) {
                      $obj_page_field->default_value = new ffData(get_word_by_code("form_" . preg_replace('/[^a-zA-Z0-9]/', '', $field_name) . "_text_" . $MD_chk["params"][0]), "Text");
