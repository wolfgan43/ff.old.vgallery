...
function EditAreaLoader(){
	...
	t.baseURL=ff.base_path + "/themes/library/editarea";
	//t.suffix="";
	t.template="";
...
EditAreaLoader.prototype ={
	...
	init : function(settings){
		...
		t.hidden= {};	// store datas of the hidden textareas
		//alert(s["id"]);
		//t.add_event(document.getElementById(s["id"]), "load", EditAreaLoader.prototype.window_loaded);
	...
	//	alert(settings["plugins"].length+": "+ settings["plugins"].join(","));
		setTimeout("editAreaLoader.get_template('"+s["id"]+"');", 50);
		//t.get_template();
