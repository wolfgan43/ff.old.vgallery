...
class uploader {
    ...
    public function __construct() {
        ...
        if ($this->config['_check4htaccess'] !== false) {
	        $htaccess = "{$this->typeDir}/.htaccess";
        ...
        if ($config['disabled'] || $config['readonly']) {
            ...
            if (!strlen($message)) {
				...
				if(function_exists("check_function") && check_function("check_fs") && function_exists("check_fs"))
				{
					...
					$file['name'] = ffCommon_url_rewrite(ffGetFilename($file['name']))
							. (pathinfo($file['name'], PATHINFO_EXTENSION)
								? "." . ffCommon_url_rewrite(pathinfo($file['name'], PATHINFO_EXTENSION))
								: ""
							);
				}
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        if (!is_dir($thumbDir) && !dir::rmkdir($thumbDir, $this->config['uploadDir'], $this->config['dirPerms']))
            return false;
