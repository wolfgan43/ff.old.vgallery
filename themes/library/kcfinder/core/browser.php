...
class browser extends uploader {
        ...
        foreach($this->post['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                if (is_file($fromThumb) && is_readable($fromThumb)) {
                    ...
                    if (!is_dir($toThumb))
                        helper::rmkdir($toThumb, $this->config['uploadDir'], $this->config['dirPerms']);
                    $toThumb .= "/$base";
    ...
    protected function getDirInfo($dir, $removable=false) {
        if ((substr(basename($dir), 0, 1) == ".") || !is_dir($dir) || !is_readable($dir) || in_array(basename($dir), $this->config['hideDir']))
            return false;
        $dirs = dir::content($dir, array('types' => "dir"));
        if (is_array($dirs)) {
            foreach ($dirs as $key => $cdir)
                if (substr(basename($cdir), 0, 1) == "." || in_array(basename($cdir), $this->config['hideDir']))
                    unset($dirs[$key]);
