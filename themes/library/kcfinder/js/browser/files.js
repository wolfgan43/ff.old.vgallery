        ...
        $.each(browser.files, function(i, file) {
            if (_.kuki.get('view') == 'list') {
                ...
                if (file.thumb)
                    ...
                    var icon = (browser.uploadURL == '/' ? '' : browser.uploadURL) + '/' + browser.dir + '/' + file.name;
...
browser.returnFile = function(file) {
    ...
    var fileURL = file.substr
        ? file : (browser.uploadURL == '/' ? '' : browser.uploadURL) + '/' + browser.dir + '/' + file.data('name');
...
browser.returnFiles = function(files) {
    if (this.opener.callBackMultiple && files.length) {
        var rfiles = [];
        $.each(files, function(i, file) {
            rfiles[i] = (browser.uploadURL == '/' ? '' : browser.uploadURL) + '/' + browser.dir + '/' + $(file).data('name');
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        $('.menu a[href="kcact:view"]').click(function() {
            ...
            var img = new Image();
            var url = _.escapeDirs((browser.uploadURL == '/' ? '' : browser.uploadURL) + '/' + path);
            img.src = url;
            img.onload = function() {
