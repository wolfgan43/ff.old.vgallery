...
function system_redirect_get_destination($hostname, $request_uri) {
	if(is_file(CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".php")) {
		$filesize = floor(filesize(CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".php") / 1000000);
...
function system_redirect_set_destination($hostname, $request_uri, $destination, $code = '') {
	...
	$file = CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".php";
...
function system_redirect_get_rule($hostname, $write_cache = false, $skip_db = false) {
	...
	$file = CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".rule.php";
	if(!$loaded_redir[$hostname]) {
		if(is_file($file)) {
			require($file);
		} else {
...
function system_redirect_get_rule_old($hostname, $write_cache = false) {
	...
	if(!is_array($loaded_redirect[$hostname])) {
		...
		if(is_file(CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".rule.php")) {
			require_once(CM_CACHE_DISK_PATH . "/redirect/" . $hostname . ".rule.php");
...
function system_redirect_set_rule($hostname, $request_uri, $destination, $code = "301") {
	...
	if($rule["ID"]) {
		@unlink(CM_CACHE_DISK_PATH . "/redirect/" . $rule["hostname"] . ".php");
    	@unlink(CM_CACHE_DISK_PATH . "/redirect/" . $rule["hostname"] . ".rule.php");
	}
