    ...
    if(isset($admin_menu["unic_name"]) && strlen($admin_menu["unic_name"])) {
       ...
       /* if(strlen($layout_settings["ADMIN_INTERFACE_PLUGIN"])) {
        ...
        if($template_name == "popup") {
            ...
            $icon_layout_size = "lg";
            $allow_delete = false;
            $tpl->set_var("block_edit_class", "modify"); //Cms::getInstance("frameworkcss")->get(null, "icon", "stack"));
            $tpl->set_var("block_edit_icon", Cms::getInstance("frameworkcss")->get("th-large", "icon-tag", $icon_layout_size)); 
            $tpl->set_var("block_delete_class", "delete"); //Cms::getInstance("frameworkcss")->get(null, "icon", "stack"));
		...
		if(isset($admin_menu["class"]) && strlen($admin_menu["class"])) { 
            $tpl->set_var("block_class", "block"); //Cms::getInstance("frameworkcss")->get("vg-" . $admin_menu["class"], "icon", array($admin_menu["group"], "2x")));
        ...
        $tpl->set_var("modify_class", "modify"); //Cms::getInstance("frameworkcss")->get("editrow", "icon", $icon_size));
        $tpl->set_var("modify_icon", Cms::getInstance("frameworkcss")->get("editrow", "icon-tag", $icon_size)); 
        $tpl->set_var("delete_class", "delete"); //Cms::getInstance("frameworkcss")->get("deleterow", "icon", $icon_size));
        ...
        if(isset($admin_menu["adddir"]) && strlen($admin_menu["adddir"])) {
            ...
            $tpl->set_var("adddir_class", "add-dir"); //Cms::getInstance("frameworkcss")->get(null, "icon", "stack"));
        ...
        if(isset($admin_menu["addnew"]) && strlen($admin_menu["addnew"])) {
            ...
            $tpl->set_var("addnew_class", "add-new"); //Cms::getInstance("frameworkcss")->get(null, "icon", "stack"));
